        ...
        <div class="row">
            <div class="col-md-6">
                <div class="f_client_id">
                    <div class="form-group select-placeholder">
                        ...
                        <select id="clientid" name="clientid" data-live-search="true" data-width="100%"
                            ...
                            <?php if ($selected != '') {
                                $rel_data = get_relation_data_invoice('customer', $selected);
                ...
                if (! isset($invoice_from_project)) { ?>
                    <div class="form-group select-placeholder projects-wrapper hide">
                        <label
                ...
                <?php
                    $_is_draft = (isset($invoice) && $invoice->status == 6);

                    if (isset($invoice)) {
                        // Edit mode
                        $formatted = format_alb_invoice_number_custom($invoice);
                        $prefix = get_alb_invoice_prefix_custom();
                        $_invoice_number = str_replace($prefix, '', $formatted);
                    } else {
                        // Create mode
                        $prefix = get_alb_invoice_prefix_custom();
                        $next_number = get_next_alb_invoice_number_custom();
                        $padding = get_alb_number_padding_custom();
                        $_invoice_number = str_pad($next_number, $padding, '0', STR_PAD_LEFT);
                    }
                ...
                <div class="form-group">
                    ...
                    <div class="input-group">
                        ...
                        <input type="text" name="number" class="form-control"
                            value="<?= ($_is_draft) ? format_alb_draft_number_custom($invoice->number) : $_invoice_number; ?>"
            ...
            <div class="col-md-6">
                <div class="tw-ml-3">
                    <div
                        class="form-group mbot15 hide<?= count($payment_modes) > 0 ? ' select-placeholder' : ''; ?>">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <?php
                            $selected = isset($invoice) ? $invoice->invoice_currency : ''; ?>
                            <?= render_select('invoice_currency', $currencies, ['id', 'name', 'symbol'], 'invoice_add_edit_currency', $selected, ['required' => true]); ?>
                        </div>
                        <div class="col-md-6">
                            <?php
        ...
        <div class="table-responsive s_table">
            <table class="table invoice-items-table items table-main-invoice-edit has-calculations no-mtop">
                <thead>
                    <tr>
                        <th></th>
                        <th width="30%" align="left"><i class="fa-solid fa-circle-exclamation tw-mr-1"
                                aria-hidden="true" data-toggle="tooltip"
                                data-title="<?= _l('item_description_new_lines_notice'); ?>"></i>
                            <?= _l('invoice_table_item_heading'); ?>
                        </th>
                        <th width="25%" align="left" class="hide">
                        ...
                        ?>
                        <th width="15%" align="right" class="qty">
                            <?= e($qty_heading); ?>
                        </th>
                        <th width="20%" align="right" class="">
                            <?= _l('unit'); ?>
                        </th>
                        <th width="20%" align="right">
                            <?= _l('invoice_table_rate_heading'); ?>
                        </th>
                        <th width="20%" align="right" class="hide">
                ...
                <tbody>
                    ...
                    <?php if (isset($invoice) || isset($add_items)) {
                        ...
                        foreach ($add_items as $item) {
                            ...
                            $table_row .= '</td>';
                            $table_row .= '<td class=" hide bold description"><textarea name="' . $items_indicator . '[' . $i . '][description]" class="form-control" rows="5">' . clear_textarea_breaks($item['description']) . '</textarea></td>';
                            ...
                            $table_row .= '<td><input type="text" placeholder="' . $unit_placeholder . '" name="' . $items_indicator . '[' . $i . '][unit]" class="form-control  text-right" value="' . $item['unit'] . '">';
                            $table_row .= '</td>';
                            ...
                            $table_row .= '<td class="taxrate hide">' . $this->misc_model->get_taxes_dropdown_template('' . $items_indicator . '[' . $i . '][taxname][]', $invoice_item_taxes, 'invoice', $item['id'], true, $manual) . '</td>';
        ...
        <div class="col-md-8 col-md-offset-4">
            <table class="table text-right">
                <tbody>
                    ...
                    <!-- <tr>
                         <td><span
                                class="bold tw-text-neutral-700"><?= _l('account_total'); ?>
                                :</span>
                        </td>
                        <td class="">

<input type="text"  name="total" class="form-control"  value="<?php echo $value = isset($invoice) ? $invoice->total : 1; ?>" style="text-align: right;">
    ...
    <div class="panel-body">
        ...
        <?= render_textarea('terms', 'bank_info', $value, [], [], 'mtop15'); ?>
    </div>
...
<?php if (is_admin() || (isset($invoice) ? staff_can('edit_al_invoice', 'd3a_albania') : staff_can('create_al_invoice', 'd3a_albania'))) { ?>
<div class="btn-bottom-toolbar text-right">
    <button type="button" class="btn-tr btn btn-default invoice-form-submit transaction-submit mright5">
        <?= _l('save'); ?>
    </button>
    <button type="button" class="btn-tr btn btn-primary invoice-form-submit save-and-send transaction-submit">
        <?= _l('save_and_send'); ?>
    </button>
</div>
<?php } ?>
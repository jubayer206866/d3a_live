...
<div id="wrapper">
    <div class="content">
        <div class="tw-max-w-4xl tw-mx-auto">
            ...
            <?php
            $action = isset($expense) && $expense
                ? admin_url('d3a_albania/expense/' . $expense->id)
                : admin_url('d3a_albania/expense');
            echo form_open_multipart($action, ['id' => 'expense-form']);
            ?>
            <div class="panel_s">
                    ...
                    <div class="tab-content">
                        <div role="tabpanel" class="tab-pane active" id="tab_expense">
                            ...
                            <?php
                                ...
                                if (isset($expense) && $expense->category) {
                                    foreach ($categories as $cat) {
                                        if ($cat['id'] == $expense->category && isset($cat['category_type'])) {
                                            $selected_category_type = $cat['category_type'];
                                            break;
                                        }
                                    }
                                }
                            ...
                            <?php if (is_admin() || get_option('staff_members_create_inline_expense_categories') == '1') {
                                echo render_select('category', $categories, ['id', 'name'], 'expense_category', $selected, ['data-none-selected-text' => _l('dropdown_non_selected_tex')]);
                            } else {
                            ...
                            <div class="form-group projects-wrapper hide">
                                ...
                                <div id="project_ajax_search_wrapper">
                                    <select name="project_id" id="project_id" class="projects ajax-search"
                                        ...
                                        data-none-selected-text="<?= _l('dropdown_non_selected_tex'); ?>">
                                        <?php 
                                        if (isset($expense) && $expense->project_id) {
                                            echo '<option value="' . $expense->project_id . '" selected>' . 
                                                e(get_project_name_by_id($expense->project_id)) . 
                                                '</option>';
                                        } 
                                        ?>
                                    </select>
                            ...
                            <div class="form-group select-placeholder">
                                <?php
                                $project_id = isset($expense) && $expense->project_id ? $expense->project_id : '';
                            ...
                            <div class="form-group select-placeholder">
                                <?php
                                $selected_staff = isset($expense) ? ($expense->staffid ?? '') : '';
                            ...
                            <?php $value2 = (isset($expense) ? $expense->amount_2 : ''); ?>
                            <?= render_input('amount_2', 'Amount 2', $value2, 'number'); ?>
...
?>
                    ...
                    <?php if (is_admin() || (isset($expense) && $expense ? staff_can('edit_al_expense', 'd3a_albania') : staff_can('create_al_expense', 'd3a_albania'))) { ?>
...
<script>
    ...
    $(function() {
        ...
        <?php if (! isset($expense)) { ?>
        $('select[], select[name="tax2"]').on('change', function() {
    ...
    function customer_init() {
        ...
        $.get(admin_url + 'd3a_albania/get_project_clients/' + project_id, function(response) {
            clientSelect.html(response);
            clientSelect.selectpicker('refresh');
        });
    }
    ...
    function toggleClientRequired() {
        ...
        if (project_id && project_id !== '') {
            ...
            clientSelect.removeAttr('required');
        }
    }
    function toggleFieldsByCategoryType() {
        var type = $('select[name="category_type"]').val();

        // Hide all first
        $('.projects-wrapper').hide();
        $('select[name="clientid"]').closest('.form-group').hide();
        $('select[name="staffid"]').closest('.form-group').hide();

        if (type === 'operational') {
            // Show only staff
            $('select[name="staffid"]').closest('.form-group').show();
        }

        if (type === 'customer') {
            // Show project & customer
            $('.projects-wrapper').show();
            $('select[name="clientid"]').closest('.form-group').show();
        }
    }

    $(function () {
        // Initial load
        toggleFieldsByCategoryType();
    ...
    function populateExpenseCategory(selectedCategoryId = null) {
        var type = $('select[name="category_type"]').val();
        var categories = <?php echo json_encode($categories_by_type); ?>;
        ...
        if(categories[type]) {
            categories[type].forEach(function(cat) {
                var selected = (selectedCategoryId && selectedCategoryId == cat.id) ? 'selected' : '';
    ...
    <?php $selected_category_id = isset($expense) && $expense->category ? $expense->category : ''; ?>
    populateExpenseCategory('<?php echo $selected_category_id; ?>');

    $('select[name="category_type"]').on('change', function() {
        populateExpenseCategory();
        toggleFieldsByCategoryType();
});       
function toggleAmount2Field() {
    var selectedCategory = $('select[name="category"]').val();
    var amountLabel = $('label[for="amount"]');
    var amount2Label = $('label[for="amount_2"]');

    if (selectedCategory == '1' || selectedCategory == '3') {
        $('#amount_2').closest('.form-group').show();

        amountLabel.text('Bought');
        amount2Label.text('Sold');
    } else {
        $('#amount_2').closest('.form-group').hide();

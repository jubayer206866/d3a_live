<script>
  ...
  function goods_receipt_change(el) {
    "use strict";
    if (el.value != '') {
      $.post(admin_url + 'purchase/goods_receipt_change/' + el.value).done(function (response) {
        response = JSON.parse(response);
        if (response) {
          ...
          $('.sum_cartons').text(response.sum_cartons);
          $('.sum_total_prices').text(response.sum_total_prices);
          $('.sum_price_total').text(response.sum_price_total);
          $('.sum_total_gross_weight').text(response.sum_total_gross_weight);
          $('.sum_total_net_weight').text(response.sum_total_net_weight);
          $('.sum_total_cbm').text(response.sum_total_cbm);

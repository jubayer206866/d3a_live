      ...
      <div class="tab-content">
         ...
         <div role="tabpanel" class="tab-pane<?php if (!$this->input->get('tab')) {
            ...
            <div class="row">
               ...
               <div class="col-md-6">
                  <?php echo render_input('vendor_code', 'vendor_code', $vendor_code, 'text'); ?>
                  ...
                  <?php } ?>
                  <div class="form-group">
                     <label for="category"><span class="text-danger">*</span><!-- Add by Rifat.......... -->
                        <?php echo _l('vendor_category'); ?>
                     </label>
                     <select name="category[]" id="category" class="selectpicker" data-live-search="true" multiple data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" required><!-- Add by Rifat.......... -->
                        <?php foreach ($vendor_categories as $vc) { ?>
                           <option value="<?php echo pur_html_entity_decode($vc['id']); ?>"
                              <?php if (isset($client) && in_array($vc['id'], explode(',', $client->category))) {
                                 echo 'selected';
                              } ?>>
                              <?php echo pur_html_entity_decode($vc['category_name']); ?>
                           </option>
                        <?php } ?>
                     </select>
                  </div>
                     ...
                     <select name="default_language" id="default_language" class="form-control selectpicker" data-none-selected-text="<?php echo _l('dropdown_non_selected_tex'); ?>">
                        ...
                        foreach ($languages as $availableLanguage) {
                              ...
                              if (isset($client)) {
                                 ...
                                 if ($availableLanguage == 'chinese') {
                                    $selected = 'selected';
                                 }
                              }
                        ?>
                              <option value="<?php echo pur_html_entity_decode($availableLanguage); ?>" <?php echo pur_html_entity_decode($selected); ?>>
                  ...
                  <?php } ?>
                  <?php
                  $store_number = (isset($client) ? $client->store_number : '');
                  echo render_input('store_number', 'store_number', $store_number);
                  ?>
                  <?php
                  ...
                  ?>
                  <div class="form-group">
                     <label for="vendor_type"><span class="text-danger">*</span><!-- Add by Rifat.......... -->
                        <?php echo _l('vendor_type'); ?>
                     </label>
                     <select name="vendor_type" id="vendor_type" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" required><!-- Add by Rifat.......... -->
                        <option value=""><?php echo _l('dropdown_non_selected_tex'); ?></option>
                        <?php foreach ($vendor_type as $type) { ?>
                           <option value="<?php echo pur_html_entity_decode($type['id']); ?>"
                              <?php if (isset($client) && in_array($type['id'], explode(',', $client->vendor_type))) {
                                 echo 'selected';
                              } ?>>
                              <?php echo pur_html_entity_decode($type['name']); ?>
                           </option>
                        <?php } ?>
                     </select>
                     <small id="vendor_type_error" class="text-danger" style="display:none;">Please select a vendor type.</small>
                  </div>
               </div>
               <div class="col-md-6">
                  ...
                  <?php // Add by Rifat..........
                  $city_value = (isset($client) ? $client->city : '');
                  $city_attrs = (isset($client) ? [] : ['autofocus' => true]);
                  $city_attrs['required'] = 'required';
                  echo render_input('city', '<span style="color:red">*</span> ' . _l('client_city'), $city_value, 'text', $city_attrs);
                  ?>
                  <div class="hide">
                  ...
                  $customer_default_country = 46; // Add By Rifat.......
...
<script>
   ...
   $('form').on('submit', function(e) {
      $('#vendor_type').selectpicker('refresh');

...
class Purchase_model extends App_Model
{
    ...
    public function add_pur_order($data)
    {
        ...
        //akash unset for add
        unset($data['koli']);
        unset($data['cope_koli']);
        unset($data['total_koli']);
        unset($data['price']);
        unset($data['price_total']);
        unset($data['net_weight']);
        unset($data['total_net_weight']);
        unset($data['gross_weight']);
        unset($data['total_gross_weight']);
        unset($data['cbm_koli']);
        unset($data['total_cbm']);
        //akash unset for add end
        ...
        if ($insert_id) {
            ...
            if (count($order_detail) > 0) {
                foreach ($order_detail as $key => $rqd) {
                    ...
                    //akash add pur_details
                    $dt_data['koli'] = $rqd['koli'];
    ...
    public function update_pur_order($data, $id)
    {
        ...
        $items = $this->get_pur_order_detail($id);
        $total_price_total=0;
        foreach($items as $item){
                $total_price_total += (float) preg_replace('/[^0-9.]/', '', $item['price_total']);
        }
 
        $data['total'] = $total_price_total;
            $data['subtotal'] = $total_price_total;
        $this->db->where('id', $id);
        ...
        $this->db->update(db_prefix() . 'goods_receipt', ['project' => $data['project']]);
    ...
    public function delete_pur_order($id)
    {
        ...
        $pur_order_details = $this->db->get(db_prefix() . 'pur_order_detail')->result_array();
        $item_ids = array();
        foreach ($pur_order_details as $pur_order_detail) {
            $item_ids[$pur_order_detail['item_code']] = $pur_order_detail['item_code'];
        }
        if (count($item_ids)) {
            $ids = array_values($item_ids);
            $this->db->where('pur_order !=', $id);
            $this->db->where_in('item_code', $ids);
            $items_in_other_po = $this->db->get(db_prefix() . 'pur_order_detail')->result_array();
            foreach ($items_in_other_po as $item_in_other_po) {
                unset($item_ids[$item_in_other_po['item_code']]);
            }
            if (count(value: $item_ids)) {
                $item_ids = array_values($item_ids);
                $this->delete_commodity_with_files($item_ids);
            }
        }
        ...
        if (count($goods_receipt)) {
            ...
            if (count($pur_invoices)) {
                $this->delete_pur_invoice($pur_invoices[0]['id']);
            }

            $this->delete_goods_receipt($goods_receipt[0]['id']);
            if ($goods_receipt[0]['goods_delivery_id']) {
                $this->db->where('delivery_note_id', $goods_receipt[0]['goods_delivery_id']);
                $wh_packing_lists = $this->db->get(db_prefix() . 'wh_packing_lists')->result_array();
                if (count($wh_packing_lists)) {
                    $this->delete_packing_list($wh_packing_lists[0]['goods_delivery_id']);
                }

                $this->delete_goods_delivery($goods_receipt[0]['goods_delivery_id']);
            }
        }
    ...
    public function delete_commodity_with_files($ids)
    {

        $this->db->where_in('commodity_id', $ids);
        $this->db->delete(db_prefix() . 'inventory_commodity_min');
        /*delete file attachment*/
        $array_file = $this->get_warehourse_attachments($ids);
    ...
    public function get_warehourse_attachments($commodity_ids)
    {

        $this->db->order_by('dateadded', 'desc');
        $this->db->where_in('rel_id', $commodity_ids);
    ...
    public function get_purorder_pdf_html($pur_order_id)
    {
        ...
        $info_right_column .= '<b style="font-size:20px; text-align:center">Purchase Order</b>';

        $html = '';
        $html .= '<table class="table">
        <tbody>
        <tr>
        	<td rowspan="1" width="50%" class="text-center">' . pdf_logo_url() . '</td>
        </tr>
        <tr>
      ...
      ';
        ...
        <thead>
          <tr>
            <th class="thead-dark" style="width: 11%; text-align:center;">Product Code</th>
            <th class="thead-dark" style="width: 11%; text-align:center;">Product Name</th>
            <th class="thead-dark" style="width: 8%; text-align:center;">Cartons</th>
            <th class="thead-dark" style="width: 7%; text-align:center;">Pieces Carton</th>
            <th class="thead-dark" style="width: 7%; text-align:center;">Total Pieces</th>
            <th class="thead-dark" style="width: 7%; text-align:center;">Price</th>
            <th class="thead-dark" style="width: 7%; text-align:center;">Price Total</th>
            <th class="thead-dark" style="width: 7%; text-align:center;">Gross Weight</th>
            <th class="thead-dark" style="width: 7%; text-align:center;">Total Gross Weight</th>
            <th class="thead-dark" style="width: 7%; text-align:center;">Net Weight</th>
            <th class="thead-dark" style="width: 7%; text-align:center;">Total Net Weight</th>
            <th class="thead-dark" style="width: 7%; text-align:center;">CBM</th>
            <th class="thead-dark" style="width: 7%; text-align:center;">Total CBM</th>';
        $html .= '</tr>
          </thead>
          <tbody>';
        $koli = 0;
        ...
        foreach ($pur_order_detail as $row) {
            $vew_price_total = (strpos($row['price_total'], '¥') !== false) ? $row['price_total'] : '¥' . $row['price_total'];

            $html .= '<tr nobr="true" class="sortable">
            <td style="width: 11%;">' . $row['item_name'] . '</td>
            <td style="width: 11%;">' . $row['description'] . '</td>
            <td style="width: 8%;">' . $row['koli'] . '</td>
            <td style="width: 7%;">' . $row['cope_koli'] . '</td>
            <td style="width: 7%;">' . $row['total_koli'] . '</td>
            <td style="width: 7%;">' . $row['price'] . '</td>
            <td style="width: 7%;">' . $vew_price_total . '</td>
            <td style="width: 7%;">' . $row['gross_weight'] . '</td>
            <td style="width: 7%;">' . $row['total_gross_weight'] . '</td>
            <td style="width: 7%;">' . $row['net_weight'] . '</td>
            <td style="width: 7%;">' . $row['total_net_weight'] . '</td>
            <td style="width: 7%;">' . $row['cbm_koli'] . '</td>
            <td style="width: 7%;">' . $row['total_cbm'] . '</td>';
            $html .= '</tr>';

            $koli += (float) $row['koli'];
            $total_koli += (float) $row['total_koli'];
    ...
    public function add_pur_invoice($data)
    {
        ...
        if ($insert_id) {
            ...
            $this->db->where('id', $data['goods_receipt_id']);
    ...
    public function delete_pur_invoice($id)
    {
        ...
        $this->db->where('pur_inv_id', $id);
        $this->db->update(db_prefix() . 'goods_receipt', ['pur_inv_id' => null]);
    ...
    public function pur_get_grouped($can_be = '', $search_all = false, $vendor = '')
    {
        ...
        foreach ($groups as $group) {
            ...
            $this->db->where(db_prefix() . 'items.group_item', 0);
    ...
    public function get_item_v2($id = '')
    {
        ...
        $this->db->select($rateCurrencyColumns . '' . db_prefix() . 'items.id as itemid,rate,
            ...
            CONCAT(commodity_code,"_",description) as code_description,long_description,group_id,' . db_prefix() . 'items_groups.name as group_name,unit,' . db_prefix() . 'ware_unit_type.unit_name as unit_name, purchase_price, unit_id, guarantee,koli,cope_koli,total_koli,rate,price_total,gross_weight,total_gross_weight,net_weight,total_net_weight,cbm_koli,total_cbm');
    ...
    public function create_purchase_order_row_template($name = '', $item_name = '', $item_description = '', $quantity = '', $unit_name = '', $unit_price = '', $taxname = '', $item_code = '', $unit_id = '', $tax_rate = '', $total_money = '', $discount = '', $discount_money = '', $total = '', $into_money = '', $tax_id = '', $tax_value = '', $item_key = '', $is_edit = false, $currency_rate = 1, $to_currency = '', $koli = '', $cope_koli = '', $total_koli = '', $rate = '', $price_total = '', $gross_weight = '', $total_gross_weight = '', $net_weight = '', $total_net_weight = '', $cbm_koli = '', $total_cbm = '')
    {
        ...
        $row .= '<td class="gross_weight">' . render_input($name_net_weight, '', $gross_weight, 'text', ['placeholder' => 'G.W']) . '</td>';
        $row .= '<td class="total_gross_weight">' . render_input($name_total_net_weight, '', $total_gross_weight, 'text', ['placeholder' => 'T.G.W']) . '</td>';
        $row .= '<td class="net_weight">' . render_input($name_gross_weight, '', $net_weight, 'text', ['placeholder' => 'N.W']) . '</td>';
        $row .= '<td class="total_net_weight">' . render_input($name_total_gross_weight, '', $total_net_weight, 'text', ['placeholder' => 'T.N.W']) . '</td>';
        ...
        $row .= '<td class="rate hide">' . render_input($name_unit_price, '', $unit_price, 'number', $array_rate_attr, [], 'no-margin', $text_right_class);
        ...
        $row .= '<td class="tax_value hide">' . render_input($name_tax_value, '', $tax_value, 'number', $array_subtotal_attr, [], '', $text_right_class) . '</td>';

        $row .= '<td class="_total hide" align="right">' . $total . '</td>';

        if ($discount_money > 0) {
            $discount = '';
        }

        $row .= '<td class="discount hide">' . render_input($name_discount, '', $discount, 'number', $array_discount_attr, [], '', $text_right_class) . '</td>';
        $row .= '<td class="discount_money hide" align="right">' . render_input($name_discount_money, '', $discount_money, 'number', $array_discount_money_attr, [], '', $text_right_class . ' item_discount_money') . '</td>';
        $row .= '<td class="label_total_after_discount hide" align="right">' . app_format_number($total_money) . '</td>';
     ...
     * @return     string      
     */
    public function create_purchase_invoice_row_template($name = '', $item_name = '', $item_description = '', $quantity = '', $unit_name = '', $unit_price = '', $taxname = '', $item_code = '', $unit_id = '', $tax_rate = '', $total_money = '', $discount = '', $discount_money = '', $total = '', $into_money = '', $tax_id = '', $tax_value = '', $item_key = '', $is_edit = false, $currency_rate = 1, $to_currency = '', $commodity_name, $commodity_long_description, $koli, $cope_koli, $total_koli, $price, $price_total, $net_weight, $total_net_weight, $gross_weight, $total_gross_weight, $cbm_koli, $total_cbm)
    {
        ...
        if ($name == '') {
            ...
            //custom
            $name_commodity_name = $name . '[item_name]';
            $name_commodity_long_description = $name . '[description]';
            $name_cartons = $name . '[koli]';
            $name_pieces_carton = $name . '[cope_koli]';
            $name_total_pieces = $name . '[total_koli]';
            $name_price = $name . '[price]';
        ...
        $row .= '<td class="commodity_name">' . render_textarea($name_commodity_name, '', $commodity_name, ['rows' => 2, 'placeholder' => _l('pur_item_name'), 'readonly' => true]) . '</td>';

        $row .= '<td class="name_commodity_name">' . render_textarea($name_commodity_long_description, '', $commodity_long_description, ['rows' => 2, 'placeholder' => _l('item_description')]) . '</td>';

        $row .= '<td class="koli">' . render_input($name_cartons, '', $koli, 'text', ['placeholder' => 'Cartons']) . '</td>';
        $row .= '<td class="cope_koli">' . render_input($name_pieces_carton, '', $cope_koli, 'text', ['placeholder' => 'Pieces/Carton']) . '</td>';
        $row .= '<td class="total_koli">' . render_input($name_total_pieces, '', $total_koli, 'text', ['placeholder' => 'Total/Pieces']) . '</td>';
        $row .= '<td class="price">' . render_input($name_price, '', $price, 'text', ['placeholder' => 'Price']) . '</td>';
        $row .= '<td class="price_total">' . render_input($name_Price_total, '', $price_total, 'text', ['placeholder' => 'Price/Total']) . '</td>';
        ...
        $row .= '<td class="hide ">' . render_textarea($name_item_code, '', $item_code, ['rows' => 2, 'placeholder' => _l('pur_item_name')]) . '</td>';

        $row .= '<td class="hide ">' . render_textarea($name_item_description, '', $item_description, ['rows' => 2, 'placeholder' => _l('item_description')]) . '</td>';

        $row .= '<td class="hide rate">' . render_input($name_unit_price, '', $unit_price, 'number', $array_rate_attr, [], 'no-margin', $text_right_class);
        if ($unit_price != '') {
            $original_price = ($currency_rate > 0) ? round(($unit_price / $currency_rate), 2) : 0;
            $base_currency = get_base_currency();
            if ($to_currency != 0 && $to_currency != $base_currency->id) {
        ...
        $row .= '<td class="hide _total" align="right">' . $total . '</td>';

        if ($discount_money > 0) {
            $discount = '';
        }

        $row .= '<td class="hide discount">' . render_input($name_discount, '', $discount, 'number', $array_discount_attr, [], '', $text_right_class) . '</td>';
        $row .= '<td class="hide discount_money" align="right">' . render_input($name_discount_money, '', $discount_money, 'number', $array_discount_money_attr, [], '', $text_right_class . ' item_discount_money') . '</td>';
        $row .= '<td class="hide label_total_after_discount" align="right">' . app_format_number($total_money) . '</td>';
    ...
    public function get_purchase_invoice_pdf_html($invoice_id)
    {
        ...
        $info_right_column = '<b style="font-size:20px; font-weight:600; text-align:center">D3A TRADING COMPANY LIMITED</b><br>';
        $info_right_column .= '<b style="font-size:20px; text-align:center">Purchase Invoice</b>';
    ...
    <thead>
      <tr>
        <th class="thead-dark">Product Code</th>
        <th class="thead-dark">Product Name</th>
        <th class="thead-dark">Cartons</th>
        <th class="thead-dark">Pieces/Carton</th>
        <th class="thead-dark">Total Pieces</th>
        <th class="thead-dark">Unit Price</th>
        <th class="thead-dark">Price Total</th>
        <th class="thead-dark">Gross Weight</th>
        <th class="thead-dark">Total Gross Weight</th>
        <th class="thead-dark">Net Weight</th>
        <th class="thead-dark">Total Net Weight</th>
        <th class="thead-dark">CBM</th>
        <th class="thead-dark">Total CBM</th>
      </tr>
    </thead>
    <tbody>';
        $sum_cartons = 0;
        ...
        foreach ($invoice_detail as $row) {
            ...
            $cartons = floatval($row['koli'] ?? 0);
            $pieces_per_carton = floatval($row['cope_koli'] ?? 0);
            $total_pieces = floatval($row['total_koli'] ?? 0);
            $unit_price = floatval($row['price'] ?? 0);
            $price_total = floatval(preg_replace('/[^0-9.]/', '', $row['price_total'] ?? 0));
            $gross_weight = floatval($row['gross_weight'] ?? 0);
            $total_gross_weight = floatval($row['total_gross_weight'] ?? 0);
            $net_weight = floatval($row['net_weight'] ?? 0);
            $total_net_weight = floatval($row['total_net_weight'] ?? 0);
            $cbm = floatval($row['cbm_koli'] ?? 0);
            $total_cbm = floatval($row['total_cbm'] ?? 0);
            $sum_cartons += $cartons;
            $sum_total_pieces += $total_pieces;
            $sum_price_total += $price_total;
            $sum_total_gross_weight += $total_gross_weight;
            $sum_total_net_weight += $total_net_weight;
            $sum_total_cbm += $total_cbm;

            $html .= '<tr nobr="true">
          <td>' . pur_html_entity_decode($product_code) . '</td>
          <td>' . pur_html_entity_decode($product_name) . '</td>
          <td>' . $cartons . '</td>
          <td>' . $pieces_per_carton . '</td>
          <td>' . $total_pieces . '</td>
          <td>' . pur_html_entity_decode($unit_price) . '</td>
          <td>' . app_format_money($price_total, $base_currency->symbol) . '</td>
          <td>' . $gross_weight . '</td>
        ...
        $html .= '<table class="table">
      <tbody>
        <tr><td></td><td></td><td></td><td colspan="2" class="text_left"><b>Cartons</b></td><td class="text_right">' . $sum_cartons . '</td></tr>
        <tr><td></td><td></td><td></td><td colspan="2" class="text_left"><b>Total Pieces</b></td><td class="text_right">' . $sum_total_pieces . '</td></tr>
        <tr><td></td><td></td><td></td><td colspan="2" class="text_left"><b>Price Total</b></td><td class="text_right">' . app_format_money($sum_price_total, $base_currency->symbol) . '</td></tr>
        <tr><td></td><td></td><td></td><td colspan="2" class="text_left"><b>Total Gross Weight</b></td><td class="text_right">' . $sum_total_gross_weight . '</td></tr>
        <tr><td></td><td></td><td></td><td colspan="2" class="text_left"><b>Total Net Weight</b></td><td class="text_right">' . $sum_total_net_weight . '</td></tr>
        <tr><td></td><td></td><td></td><td colspan="2" class="text_left"><b>Total CBM</b></td><td class="text_right">' . $sum_total_cbm . '</td></tr>
      </tbody>
    </table>';
        if ($invoice->terms != '') {
            $html .= '<div class="col-md-12 mtop15">
    ...
    public function get_inventory_receives_for_inv($add = false)
    {
        if ($add) {
            $this->db->where('pur_inv_id is null');
        }
    ...
    public function get_irv_items($project_id)
    {

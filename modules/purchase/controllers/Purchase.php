...
class purchase extends AdminController
{
    ...
    public function vendor($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                if (isset($post['country'])) {
                    $post['billing_country'] = $post['country'];
                    $post['shipping_country'] = $post['country'];
                }

                $success = $this->purchase_model->update_vendor($post, $id);
                if ($success == true) {
        ...
        if ($id == '') {
            ...
            $data['tab'][] = ['name' => 'purchase_order', 'icon' => '<i class="fa fa-cfsart-plus menu-icon"></i>'];
        ...
        $query = $this->db->get(db_prefix() . 'pur_vendor');
        $last_id = $query->row()->userid;
        $next_id = $last_id + 1;
        $data['vendor_code'] = isset($client) ? $client->vendor_code : 'VEN:' . $next_id;
    ...
    public function pur_order($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if (isset($_FILES['file_csv']['name']) && $_FILES['file_csv']['name'] != '') {
                ...
                $filePath = $_FILES['file_csv']['tmp_name'];

                $spreadsheet = IOFactory::load($filePath);
                $sheet = $spreadsheet->getActiveSheet();
                $files = array();
                foreach ($sheet->getDrawingCollection() as $row) {
                    $Coordinates = substr($row->getCoordinates(), 1);

                    if ($row instanceof Drawing) {
                        $path = $row->getPath();
                        $files[$Coordinates]['orginal_path'] = $path;
                        $files[$Coordinates]['fileType'] = pathinfo($path, PATHINFO_EXTENSION);

                    } elseif ($row instanceof MemoryDrawing) {
                        ob_start();
                        call_user_func($row->getRenderingFunction(), $row->getImageResource());
                        ...
                        $imageType = $mimeMap[$mime] ?? IMAGETYPE_PNG;
                        $extension = image_type_to_extension($imageType, false);
                        $filename = tempnam(sys_get_temp_dir(), 'mem_img_') . '.' . $extension;
                        file_put_contents($filename, $imageContents);
                        $files[$Coordinates]['orginal_path'] = $filename;
                        $files[$Coordinates]['fileType'] = $extension;
                    }
                }
                //image in xlsx

                $data = $sheet->toArray(null, true, true, true); // keys A-Z
                $total_po_value = 0;
                ...
                foreach ($data as $rowIndex => $row) {
                    if ($rowIndex < 2 || $row['C'] == null)
                        continue;

                    $rd = [];

                    $rd['commodity_barcode'] = $row['A'] ?? '';
                    $commonVal = $row['C'] ?? '';
                    $rd['description'] = $commonVal;
                    $rd['sku_code'] = $commonVal;
                    $rd['commodity_code'] = $commonVal;
                    $rd['sku_name'] = $commonVal;
                    $rd['long_description'] = $row['D'] ?? '';
                    $rd['koli'] = 0;
                    $rd['cope_koli'] = $row['F'] ?? '';
                    $rd['total_koli'] = is_numeric($row['G']) ? (float) $row['G'] : 0;
                    $rate = preg_replace('/[^0-9.]/', '', $row['H']); // [7]
                    $rd['rate'] = (float) $rate;
                    $price_total = preg_replace('/[^0-9.]/', '', $row['I']); // [7]
                    $total_po_value = $total_po_value + $price_total;
                    $rd['price_total'] = '¥' . (float) $price_total;

                    $rd['gross_weight'] = $row['J'] ?? '';
                    $rd['total_gross_weight'] = $row['K'] ?? '';

                    $rd['net_weight'] = $row['L'] ?? '';
                    $rd['total_net_weight'] = $row['M'] ?? '';
                    ...
                    $insert_data['order'] = $rowIndex;
                    $insert_data['item_name'] = $rd['description'];
                    $insert_data['item_description'] = $rd['long_description'];
                    $insert_data['koli'] = $row['E'];
                    $insert_data['cope_koli'] = $rd['cope_koli'];
                    $insert_data['total_koli'] = $rd['total_koli'];
                    $insert_data['price'] = $rd['rate'];
                    $insert_data['price_total'] = $rd['price_total'];
                    $insert_data['net_weight'] = $rd['net_weight'];
                    $insert_data['total_net_weight'] = $rd['total_net_weight'];
                    $insert_data['gross_weight'] = $rd['gross_weight'];
                    $insert_data['total_gross_weight'] = $rd['total_gross_weight'];
                    $insert_data['cbm_koli'] = $rd['cbm_koli'];
                    $insert_data['total_cbm'] = $rd['total_cbm'];
                    $insert_data['quantity'] = 1;
                    $insert_data['id'] = '';
                    $insert_data['tax_value'] = '';
                    $insert_data['discount'] = 1;
                    $insert_data['discount_money'] = 1;
                    $insert_data['item_code'] = 1;
                    $insert_data['unit_id'] = '';
                    $insert_data['total'] = '';
                    $insert_data['total_money'] = 0;
                    $insert_data['into_money'] = 0;

                    $flag_insert = false;
                    if (get_staff_user_id() != '') {
                        $rows[] = $rd;
                        $result_value = $this->warehouse_model->import_xlsx_commodity($rd, false);
                        if ($result_value['insert_id']) {
                            $total_rows_actualy++;
                            $flag_insert = true;
                            if (isset($result_value['insert_id'])) {
                                $insert_data['item_code'] = $result_value['insert_id'];
                                $items[$rowIndex] = $insert_data;
                                $flag_insert_id = $result_value['insert_id'];
                                $insert_ids[] = $result_value['insert_id'];
                                if (isset($files[$rowIndex])) {
                                    $folderPath = FCPATH . 'modules/purchase/uploads/item_img/' . $result_value['insert_id'] . '/';
                                    if (!file_exists($folderPath)) {
                                        mkdir($folderPath, 0755, true);
                                    }
                                    $path = $files[$rowIndex]['orginal_path'];
                                    $imageName = basename($path);
                                    $destination = $folderPath . $imageName;
                                    $image_data['rel_id'] = $result_value['insert_id'];
                                    $image_data['rel_type'] = 'commodity_item_file';
                                    $image_data['file_name'] = $imageName;
                                    $image_data['filetype'] = $files[$rowIndex + 1]['fileType'];
                                    if (copy($path, $destination)) {
                                        $this->db->insert(db_prefix() . 'files', $image_data);
                                    }
                                }
                            } else {
                                $flag_insert_id = 0;
                ...
                if (count($insert_ids) && $post_data['vendor']) {
                    $this->db->where('vendor', $post_data['vendor']);
                    $this->db->where_in('items', $insert_ids);
                    $saved_vendor_items = $this->db->get(db_prefix() . 'pur_vendor_items')->result_array();
                    $saved_vendor_item_ids = array();
                    foreach ($saved_vendor_items as $saved_vendor_item) {
                        $saved_vendor_item_ids[] = $saved_vendor_item['items'];
                    }
                    $new_items = array_diff($insert_ids, $saved_vendor_item_ids);
                    if (count($new_items)) {
                        $vendor_item_data['vendor'] = $post_data['vendor'];
                        $vendor_item_data['items'] = $new_items;
                        $this->purchase_model->add_vendor_items($vendor_item_data);
                    }
                }

            }
            $pur_order_data = $this->input->post();
            unset($pur_order_data['cartons']);
            unset($pur_order_data['pieces_carton']);
            unset($pur_order_data['total_pieces']);
            unset($pur_order_data['cbm']);
            unset($pur_order_data['price']);
            unset($pur_order_data['price_total']);
            unset($pur_order_data['net_weight']);
            unset($pur_order_data['total_net_weight']);
            unset($pur_order_data['gross_weight']);
            unset($pur_order_data['total_gross_weight']);
            unset($pur_order_data['total_cbm']);


            if (isset($pur_order_data['newitems']) && count($pur_order_data['newitems'])) {
                $pur_order_data['newitems'] = array_merge($pur_order_data['newitems'], $items);
            } else {
                $pur_order_data['newitems'] = $items;
            }
            $pur_order_data['total'] = $total_po_value;
            $pur_order_data['subtotal'] = $total_po_value;
            ...
            if ($id == '') {
                ...
                //direct inventory receipt create

                $supplier_name = $this->purchase_model->get_vendor($pur_order_data['vendor']);
                $inv_receive_data['pr_order_id'] = $id;
                $inv_receive_data['date_c'] = to_sql_date($pur_order_data['order_date']);
                $inv_receive_data['date_add'] = to_sql_date(date('Y-m-d'));
                $inv_receive_data['supplier_name'] = isset($supplier_name) ? $supplier_name->company : '';
                $inv_receive_data['buyer_id'] = $pur_order_data['buyer'];
                $inv_receive_data['description'] = $pur_order_data['vendornote'];
                $inv_receive_data['addedfrom'] = get_staff_user_id();
                $inv_receive_data['supplier_code'] = $pur_order_data['vendor'];
                $inv_receive_data['project'] = $pur_order_data['project'];
                $inv_receive_data['goods_receipt_code'] = get_warehouse_option('inventory_received_number_prefix') . get_warehouse_option('next_inventory_received_mumber');
                $inv_receive_data['total_tax_money'] = 0;
                $inv_receive_data['total_goods_money'] = 0;
                $inv_receive_data['value_of_inventory'] = 0;
                $inv_receive_data['total_money'] = 0;
                $inv_receive_data['total_results'] = count($pur_order_data['newitems']);
                $inv_receive_data['goods_receipt_detail'] = $pur_order_data['newitems'];
                $this->load->model('warehouse/warehouse_model');
                $receive_id = $this->warehouse_model->add_goods_receipt_from_purchase_order($inv_receive_data);
                //direct inventory receipt create end

                //direct inventroy delivery create
                $inv_delivery_data['goods_receipt_id'] = $receive_id;
                $inv_delivery_data['customer_code'] = $pur_order_data['clients'][0];
                $inv_delivery_data['date_c'] = to_sql_date(date('Y-m-d'));
                $inv_delivery_data['date_add'] == to_sql_date($pur_order_data['order_date']);
                $inv_delivery_data['goods_delivery_code'] = $this->warehouse_model->create_goods_delivery_code();
                $inv_delivery_data['total_money'] = 0;
                $inv_delivery_data['approval'] = 1;
                $inv_delivery_data['project'] = $pur_order_data['project'];
                $inv_delivery_data['pr_order_id'] = $id;
                $inv_delivery_data['type'] = 'total';
                $this->db->insert(db_prefix() . 'goods_delivery', $inv_delivery_data);
                $goods_delivery_id = $this->db->insert_id();
                ...
                $items_for_delivery = array();
                foreach ($pur_order_data['newitems'] as $single_item) {
                    $item_for_delivery['goods_delivery_id'] = $goods_delivery_id;
                    $item_for_delivery['commodity_code'] = $single_item['item_code'];
                    $item_for_delivery['commodity_long_description'] = $single_item['item_description'];
                    $item_for_delivery['commodity_name'] = $single_item['item_name'];
                    $item_for_delivery['warehouse_id'] = 1;
                    $item_for_delivery['quantities'] = 1;
                    $item_for_delivery['available_quantity'] = 1;
                    $item_for_delivery['koli'] = 0;
                    $item_for_delivery['cope_koli'] = $single_item['cope_koli'];
                    $item_for_delivery['total_koli'] = $single_item['total_koli'];
                    $item_for_delivery['price'] = $single_item['price'];
                    $item_for_delivery['price_total'] = $single_item['price_total'];
                    $item_for_delivery['net_weight'] = $single_item['net_weight'];
                    $item_for_delivery['total_net_weight'] = $single_item['total_net_weight'];
                    $item_for_delivery['gross_weight'] = $single_item['gross_weight'];
                    $item_for_delivery['total_gross_weight'] = $single_item['total_gross_weight'];
                    $item_for_delivery['cbm_koli'] = $single_item['cbm_koli'];
                    $item_for_delivery['total_cbm'] = $single_item['total_cbm'];
                    $items_for_delivery[] = $item_for_delivery;
                }
                $this->db->insert_batch('tblgoods_delivery_detail', $items_for_delivery);

                $this->db->where('goods_delivery_id', $goods_delivery_id);
                $saved_goods_delivery_details = $this->db->get(db_prefix() . 'goods_delivery_detail')->result_array();
                $filter_saved_goods_delivery_details = array();
                foreach ($saved_goods_delivery_details as $saved_goods_delivery_detail) {
                    $filter_saved_goods_delivery_details[$saved_goods_delivery_detail['commodity_code']] = $saved_goods_delivery_detail['id'];
                }

                //update receipt after create delivery
                $update_receipt['goods_delivery_id'] = $goods_delivery_id;
                $this->db->where('id', $receive_id);
                $this->db->update(db_prefix() . 'goods_receipt', $update_receipt);

                //packing list added
                $packing_list_data['delivery_note_id'] = $goods_delivery_id;
                $packing_list_data['packing_list_number'] = $this->warehouse_model->create_packing_list_code();
                $packing_list_data['packing_list_name'] = 'PACKING_LIST' . date('YmdHi');
                $packing_list_data['clientid'] = $pur_order_data['clients'][0];
                $packing_list_data['subtotal'] = 0;
                $packing_list_data['total_amount'] = 0;
                $packing_list_data['total_after_discount'] = 0;
                $packing_list_data['approval'] = 1;
                $packing_list_data['staff_id'] = get_staff_user_id();
                $packing_list_data['type_of_packing_list'] = 'total';
                $packing_list_data['delivery_status'] = 'wh_ready_to_deliver';
                $packing_list_data['datecreated'] = _dt(date('Y-m-d h:i:s'));
                $this->db->insert(db_prefix() . 'wh_packing_lists', $packing_list_data);
                $packing_list_id = $this->db->insert_id();

                $this->warehouse_model->update_inventory_setting(['next_packing_list_number' => (int) get_warehouse_option('next_packing_list_number') + 1]);

                $tblgoods_delivery_invoices_pr_orders_data['rel_id'] = $packing_list_id;
                $tblgoods_delivery_invoices_pr_orders_data['rel_type'] = $goods_delivery_id;
                $tblgoods_delivery_invoices_pr_orders_data['type'] = 'goods_delivery';
                $this->db->insert(db_prefix() . 'goods_delivery_invoices_pr_orders', $tblgoods_delivery_invoices_pr_orders_data);


                $items_for_packing_list = array();
                foreach ($pur_order_data['newitems'] as $single_item) {
                    $items_for_packing_list['packing_list_id'] = $packing_list_id;
                    $items_for_packing_list['commodity_code'] = $single_item['item_code'];
                    $items_for_packing_list['delivery_detail_id'] = $filter_saved_goods_delivery_details[$single_item['item_code']];
                    $items_for_packing_list['commodity_long_description'] = $single_item['item_description'];
                    $items_for_packing_list['commodity_name'] = $single_item['item_name'];
                    $items_for_packing_list['quantity'] = 1;
                    $items_for_packing_list['koli'] = 0;
                    $items_for_packing_list['cope_koli'] = $single_item['cope_koli'];
                    $items_for_packing_list['total_koli'] = $single_item['total_koli'];
                    $items_for_packing_list['price'] = $single_item['price'];
                    $items_for_packing_list['price_total'] = $single_item['price_total'];
                    $items_for_packing_list['net_weight'] = $single_item['net_weight'];
                    $items_for_packing_list['total_net_weight'] = $single_item['total_net_weight'];
                    $items_for_packing_list['gross_weight'] = $single_item['gross_weight'];
                    $items_for_packing_list['total_gross_weight'] = $single_item['total_gross_weight'];
                    $items_for_packing_list['cbm_koli'] = $single_item['cbm_koli'];
                    $items_for_packing_list['total_cbm'] = $single_item['total_cbm'];
                    $items_for_packing_lists[] = $items_for_packing_list;
                }
                $this->db->insert_batch('tblwh_packing_list_details', $items_for_packing_lists);
        ...
        if ($id == '') {
            ...
            if (count($data['pur_order_detail']) > 0) {
                ...
                foreach ($data['pur_order_detail'] as $order_detail) {
                    ...
                    //akash po edit
                    $koli = $order_detail['koli'];
                    $cope_koli = $order_detail['cope_koli'];
                    $total_koli = $order_detail['total_koli'];
                    $price = $order_detail['price'];
                    $price_total = $order_detail['price_total'];
                    $gross_weight = $order_detail['gross_weight'];
                    $total_gross_weight = $order_detail['total_gross_weight'];
                    $net_weight = $order_detail['net_weight'];
                    $total_net_weight = $order_detail['total_net_weight'];
                    $cbm_koli = $order_detail['cbm_koli'];
                    $total_cbm = $order_detail['total_cbm'];
                    $cartons += (float) $order_detail['koli'];
                    $Total_net_weight += (float) $order_detail['total_net_weight'];
                    $totalPieces += (float) $order_detail['total_koli'];
                    $Total_gross_weight += (float) $order_detail['total_gross_weight'];
                    $Pricetotal += (float) preg_replace('/[^0-9.]/', '', $order_detail['price_total']);
                    $Total_cbm += (float) $order_detail['total_cbm'];
                    //akash po edit end
                    $pur_order_row_template .= $this->purchase_model->create_purchase_order_row_template('items[' . $index_order . ']', $item_name, $order_detail['description'], $order_detail['quantity'], $unit_name, $order_detail['unit_price'], $taxname, $order_detail['item_code'], $order_detail['unit_id'], $order_detail['tax_rate'], $order_detail['total_money'], $order_detail['discount_%'], $order_detail['discount_money'], $order_detail['total'], $order_detail['into_money'], $order_detail['tax'], $order_detail['tax_value'], $order_detail['id'], true, $currency_rate, $to_currency, $koli, $cope_koli, $total_koli, $price, $price_total, $gross_weight, $total_gross_weight, $net_weight, $total_net_weight, $cbm_koli, $total_cbm);
                }
            }
    ...
    public function delete_pur_order($id)
    {
        ...
        if(count($goods_receipt)){
            $this->purchase_model->delete_goods_receipt($goods_receipt[0]['id']);
            if($goods_receipt[0]['goods_delivery_id']){
                  $this->purchase_model->delete_goods_delivery($goods_receipt[0]['goods_delivery_id']);
            }
        }
            */
    ...
    public function invoices()
    {
        ...
        $data['clients'] = $this->db
            ...
            ->result_array();
        //Payment statuses.......
        $data['payment_status'] = array_merge(
            [['payment_status' => 'all']],
            $this->db
                ->distinct()
    ...
    public function pur_invoice($id = '')
    {
        ...
        $sum_total_prices = 0;
        $sum_price_total = 0;
        $sum_total_gross_weight = 0;
        $sum_total_net_weight = 0;
        $sum_total_cbm = 0;
        if ($id != '') {
            $data['pur_orders'] = $this->purchase_model->get_pur_order_approved();
            ...
            if (count($data['pur_invoice_detail']) > 0) {
                ...
                foreach ($data['pur_invoice_detail'] as $inv_detail) {
                    ...
                    $sum_cartons += $inv_detail['koli'];
                    $sum_total_prices += (float) $inv_detail['price'];
                    $sum_price_total += (float) preg_replace('/[^0-9.]/', '', $inv_detail['price_total']);
                    $sum_total_gross_weight += (float) $inv_detail['total_gross_weight'];
                    $sum_total_net_weight += (float) $inv_detail['total_net_weight'];
                    $sum_total_cbm += (float) $inv_detail['total_cbm'];
                    if (strlen($item_name) == 0) {
                        $item_name = pur_get_item_variatiom($inv_detail['item_code']);
                    }

                    $pur_invoice_row_template .= $this->purchase_model->create_purchase_invoice_row_template('items[' . $index_order . ']', $item_name, $inv_detail['description'], $inv_detail['quantity'], $unit_name, $inv_detail['unit_price'], $taxname, $inv_detail['item_code'], $inv_detail['unit_id'], $inv_detail['tax_rate'], $inv_detail['total_money'], $inv_detail['discount_percent'], $inv_detail['discount_money'], $inv_detail['total'], $inv_detail['into_money'], $inv_detail['tax'], $inv_detail['tax_value'], $inv_detail['id'], true, $currency_rate, $to_currency, $inv_detail['item_name'], $inv_detail['description'], $inv_detail['koli'], $inv_detail['cope_koli'], $inv_detail['total_koli'], $inv_detail['price'], $inv_detail['price_total'], $inv_detail['net_weight'], $inv_detail['total_net_weight'], $inv_detail['gross_weight'], $inv_detail['total_gross_weight'], $inv_detail['cbm_koli'], $inv_detail['total_cbm']);
                }


            }
            $data['inventory_receives'] = $this->purchase_model->get_inventory_receives_for_inv();
        } else {
            $data['pur_orders'] = $this->purchase_model->get_pur_order_approved_for_inv();
            $data['inventory_receives'] = $this->purchase_model->get_inventory_receives_for_inv('add');
        }


        $data['pur_order_row_template'] = $pur_invoice_row_template;
        $data['ajaxItems'] = false;
        if (total_rows(db_prefix() . 'items') <= ajax_on_total_items()) {
            $data['items'] = $this->purchase_model->pur_get_grouped('can_be_purchased');
        } else {
        ...
        $data['sum_cartons'] = $sum_cartons;
        $data['sum_total_prices'] = $sum_total_prices;
        $data['sum_price_total'] = $sum_price_total;
        $data['sum_total_gross_weight'] = $sum_total_gross_weight;
        $data['sum_total_net_weight'] = $sum_total_net_weight;
        $data['sum_total_cbm'] = $sum_total_cbm;
    ...
    {
        $this->db->select('pr_order_id, supplier_code, pur_inv_id');
        $this->db->where('id', $id);
        ...
        if ($goods_receipt) {
            ...
            if ($status == 1) {
                if (!$goods_receipt->pur_inv_id) {
                    $prefix = get_purchase_option('pur_inv_prefix');
                    $next_number = get_purchase_option('next_inv_number');

                    $pur_data = [];
                    $pur_data['number'] = $next_number;
                    $pur_data['vendor_invoice_number'] = $pur_data['invoice_number'] = $prefix . str_pad($next_number, 5, '0', STR_PAD_LEFT);
                    $pur_data['vendor'] = $goods_receipt->supplier_code;
                    $pur_data['contract'] = null;
                    $pur_data['goods_receipt_id'] = $id;
                    $pur_data['invoice_date'] = date('Y-m-d');
                    $pur_data['duedate'] = date('Y-m-d');
                    $pur_data['currency'] = 3;
                    $pur_data['from_currency'] = 3;
                    $pur_data['to_currency'] = 3;
                    $pur_data['currency_rate'] = 1;
                    $pur_data['add_from'] = get_staff_user_id();
                    $pur_data['add_from_type'] = 'admin';
                    $pur_data['date_add'] = date('Y-m-d');
                    $pur_data['payment_status'] = 'unpaid';
                    $this->db->insert(db_prefix() . 'pur_invoices', $pur_data);
                    ...
                    if ($insert_id) {
                        ...
                        $this->db->where('id', $id);
                        $this->db->update(db_prefix() . 'goods_receipt', ['pur_inv_id' => $insert_id]);

                        $this->db->where('pur_invoice', $insert_id);
                        $this->db->delete(db_prefix() . 'pur_invoice_details');

                        $goods_receipt_details = $this->warehouse_model->get_goods_receipt_detail($id);
                        $total['subtotal'] = 0;

                        foreach ($goods_receipt_details as $rqd) {
                            $dt_data = [];
                            $dt_data['pur_invoice'] = $insert_id;
                            $dt_data['item_code'] = $rqd['commodity_code'];
                            $dt_data['unit_id'] = $rqd['unit_id'] ?? null;
                            $dt_data['unit_price'] = $rqd['unit_price'];
                            $dt_data['total'] = $rqd['sub_total'];
                            $dt_data['tax_value'] = $rqd['tax_rate'];
                            $dt_data['item_name'] = $rqd['commodity_name'];
                            $dt_data['description'] = nl2br($rqd['commodity_long_description']);
                            $dt_data['total_money'] = $rqd['sub_total'];
                            $dt_data['discount_money'] = 0;
                            $dt_data['discount_percent'] = 0;
                            $dt_data['koli'] = $rqd['checkd_koli'];
                            $dt_data['cope_koli'] = $rqd['cope_koli'];
                            $dt_data['total_koli'] = $rqd['total_koli'];
                            $dt_data['price'] = $rqd['price'];
                            $dt_data['price_total'] = $rqd['price_total'];
                            $dt_data['gross_weight'] = $rqd['gross_weight'];
                            $dt_data['total_gross_weight'] = $rqd['total_gross_weight'];
                            $dt_data['net_weight'] = $rqd['net_weight'];
                            $dt_data['total_net_weight'] = $rqd['total_net_weight'];
                            $dt_data['cbm_koli'] = $rqd['cbm_koli'];
                            $dt_data['total_cbm'] = $rqd['total_cbm'];
                            $dt_data['quantity'] = $rqd['quantity'] ?: 0;

                            $total['subtotal'] += preg_replace('/[^0-9.]/', '', $rqd['price_total']);
                        ...
                        $this->db->where('id', $goods_receipt->pr_order_id);
                        $pur_order = $this->db->get(db_prefix() . 'pur_orders')->row();
                        $deposit = $pur_order->deposit ?? 0;

                        if ($deposit > 0) {
                            $payment_data = [
                                'pur_invoice' => $insert_id,
                                'amount' => $deposit,
                                'date' => $pur_order->order_date,
                                'paymentmode' => '',
                                'transactionid' => '',
                                'note' => 'Auto payment from deposit on invoice creation',
                                'daterecorded' => date('Y-m-d H:i:s'),
                                'approval_status' => 2,
                                'requester' => get_staff_user_id(),
                            ];
                            $this->load->model('purchase_model');
                            $this->purchase_model->add_invoice_payment($payment_data, $insert_id);
                        }
                    }
                }
            }
        ...
        $status_str = _l('waiting_to_receive');
        $class = 'label-danger';

        $next_status = 1;
        $next_status_label = _l('received');

        if ($status == 1) {
            $status_str = _l('received');
            $class = 'label-success';

            $next_status = 0;
            $next_status_label = _l('waiting_to_receive');

        } elseif ($status == 2) {
            $status_str = _l('cancelled');
            $class = 'label-default';
        ...
        echo json_encode([
            'success' => $success,
            'mess' => $success ? _l('change_delivery_status_success') : _l('change_delivery_status_fail'),
            'status_str' => $status_str,
            'class' => $class,
            'html' => '<div class="dropdown inline-block mleft5 table-export-exclude">
                            <a href="#" class="dropdown-toggle text-dark" id="tableDeliveryStatus-' . $id . '" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <span data-toggle="tooltip" title="' . _l('ticket_single_change_status') . '">
                                <i class="fa fa-caret-down" aria-hidden="true"></i>
                            </span></a>
                            <ul class="dropdown-menu dropdown-menu-right" aria-labelledby="tableDeliveryStatus-' . $id . '">

                                <li>
                                    <a href="#" onclick="change_delivery_status(1,' . $id . '); return false;">
                                        ' . _l('received') . '
                                    </a>
                                </li>

                                <li>
                                    <a href="#" onclick="change_delivery_status(0,' . $id . '); return false;">
                                        ' . _l('waiting_to_receive') . '
                                    </a>
                                </li>

                                <li>
                                    <a href="#" onclick="change_delivery_status(2,' . $id . '); return false;">
                                        ' . _l('cancelled') . '
                                    </a>
                                </li>
    ...
    public function validate_debit_note_number()
    {
        ...
        if (
            total_rows(db_prefix() . 'pur_debit_notes', [
                'YEAR(date)' => date('Y', strtotime(to_sql_date($date))),
                'number' => $number,
            ]) > 0
        ) {
    ...
    public function get_purchase_order_row_template()
    {
        ...
        $cope_koli = $this->input->post('cope_koli');
        ...
        $cbm_koli = $this->input->post('cbm_koli');
        $total_cbm = $this->input->post('total_cbm');
        $price = $this->input->post('price');

        echo $this->purchase_model->create_purchase_order_row_template($name, $item_name, $item_description, $quantity, $unit_name, $unit_price, $taxname, $item_code, $unit_id, $tax_rate, '', $discount, '', '', '', '', '', $item_key, false, $currency_rate, $to_currency, $koli, $cope_koli, $total_koli, $price, $price_total, $gross_weight, $total_gross_weight, $net_weight, $total_net_weight, $cbm_koli, $total_cbm);
    }
    ...
    public function print_qrcode($id)
    {
        ...
        $data['items'] = $this->db->get(db_prefix() . 'pur_order_detail')->result_array();
        foreach ($data['items'] as $value) {
            $barcode_file = WAREHOUSE_PRINT_ITEM . md5(trim($value['commodity_barcode'])) . '.svg';
            $barcode_file_full = FCPATH . $barcode_file;

            // Check if file exists and is valid (not empty, has barcode content)
            $needs_regeneration = true;
            if (file_exists($barcode_file_full)) {
                $file_content = file_get_contents($barcode_file_full);
                // Check if file has valid SVG content with barcode bars
                if ($file_content && strlen(trim($file_content)) > 200 && strpos($file_content, '<rect') !== false) {
                    $needs_regeneration = false;
                } else {
                    // File exists but is empty or invalid, delete it
                    @unlink($barcode_file_full);
                }
            }

            if ($needs_regeneration) {
                $this->warehouse_model->getBarcode($value['commodity_barcode']);
            }
        }
        $this->db->where('id', $id);
        $this->db->join(db_prefix() . 'clients', '' . db_prefix() . 'clients.userid = ' . db_prefix() . 'pur_orders.clients', 'left');
        $data['details'] = $this->db->get(db_prefix() . 'pur_orders')->result_array();
        ...
        foreach ($data['items'] as $item) {
            ...
            $this->db->where('commodity_code', $item['item_code']);
            $goods_receipt_detail = $this->db->get(db_prefix() . 'goods_receipt_detail')->result_array();
            
            $url = site_url("admin/warehouse/change_goods_receipt_detail/" . $goods_receipt_detail[0]['id']);
            $qrCode = new QrCode(
                data: $url,
            ...
            $Path = FCPATH . 'modules/purchase/uploads/qr_code/' . $name;
            $result->saveToFile($Path);
        }
        $data['arr_images'] = $this->warehouse_model->item_attachments();

        $this->load->view('qrcode', $data);
    }
    public function xxprint_qrcode($id)
    {
        $this->db->where('pur_order', $id);
        $data['items'] = $this->db->get(db_prefix() . 'pur_order_detail')->result_array();
        ...
        foreach ($data['items'] as $item) {
            $var = sprintf(
                "Product Code: %s\nProduct Name: %s\nCartons: %s\nPieces/Carton: %s\nTotal/Pieces: %s\nPrice: %s\nPrices Total: %s\nNW: %s\nTotal NW: %s\nGW: %s\nTotal GW: %s\nCBM: %s\nTotal CBM: %s",
                htmlspecialchars($item['item_name'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['description'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['koli'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['cope_koli'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['total_koli'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['price'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['price_total'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['net_weight'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['total_net_weight'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['gross_weight'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['total_gross_weight'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['cbm_koli'] ?? 'N/A', ENT_QUOTES, 'UTF-8'),
                htmlspecialchars($item['total_cbm'] ?? 'N/A', ENT_QUOTES, 'UTF-8')
            );
            $qrCode = new QrCode(
                data: $var,
                encoding: new Encoding('UTF-8'),
    ...
    public function goods_receipt_change($ct)
    {
        $this->load->model('warehouse/warehouse_model');
        $goods_receipt = $this->warehouse_model->get_goods_receipt($ct);

        $goods_receipt_details = $this->warehouse_model->get_goods_receipt_detail($ct);
        $sum_cartons = 0;
        $sum_total_prices = 0;
        $sum_price_total = 0;
        $sum_total_gross_weight = 0;
        ...
        if (count($goods_receipt_details) > 0) {
            $index = 0;
            foreach ($goods_receipt_details as $key => $item) {
                $index++;
                $unit_name = pur_get_unit_name($item['unit_id']);
                $taxname = $item['tax_name'];
                $item_name = $item['item_name'];
                ...
                $sum_cartons += $item['koli'];
                $sum_total_prices += (float) $item['price'];
                $sum_price_total += (float) preg_replace('/[^0-9.]/', '', $item['price_total']);
                $sum_total_gross_weight += (float) $item['total_gross_weight'];
                $sum_total_net_weight += (float) $item['total_net_weight'];
                $sum_total_cbm += (float) $item['total_cbm'];
                $list_item .= $this->purchase_model->create_purchase_invoice_row_template('newitems[' . $index . ']', $item_name, '', $item['quantity'], $unit_name, $item['unit_price'], $taxname, $item['item_code'], $item['unit_id'], $item['tax_rate'], $item['total_money'], $item['discount_%'], $item['discount_money'], $item['total'], $item['into_money'], $item['tax'], $item['tax_value'], $index, true, '', '', $item['commodity_name'], $item['commodity_long_description'], $item['koli'], $item['cope_koli'], $item['total_koli'], $item['price'], $item['price_total'], $item['net_weight'], $item['total_net_weight'], $item['gross_weight'], $item['total_gross_weight'], $item['cbm_koli'], $item['total_cbm']);
            }
        }
        $pur_order = $this->purchase_model->get_pur_order($goods_receipt->pr_order_id);



        echo json_encode([
            'list_item' => $list_item,
            'sum_cartons' => $sum_cartons,
            'sum_total_prices' => $sum_total_prices,
            'sum_price_total' => $sum_price_total,
            'sum_total_gross_weight' => $sum_total_gross_weight,
            'sum_total_net_weight' => $sum_total_net_weight,
            'sum_total_cbm' => $sum_total_cbm,
            'vendor' => $pur_order->vendor,
        ]);
    }

    public function change_po_delivery_status()
    {
        if (!$this->input->is_ajax_request()) {
            show_404();
        }
        ...
        $this->db->where('id', $id);
        $updated = $this->db->update(db_prefix() . 'pur_orders', [

...
class Whatsapp_api_lib
{
    ...
    public function send_mapped_template($category, ...$params)
    {
        ...
        foreach ($all_templates as $template) {
            ...
            if ($category == "project_status_changed") {
                $projectID = $params[array_key_first($params)];
                $this->merge_fields['{project_link}'] = admin_url('projects/view/' . $projectID);
                if ($template->send_to == 'contact') {
                    $this->merge_fields['{project_link}'] = site_url('clients/project/' . $projectID);
                }
            }
    ...
    {
        // Disabled - return early to prevent PDF generation error
        return ['status' => false, 'message' => 'Invoice WhatsApp notification disabled'];
    ...
    public function project_status_changed($projectID, $statusID = null)
    {
        $this->CI->load->model('projects_model');
        $tableData       = $this->CI->projects_model->get($projectID);
        $members         = $this->CI->projects_model->get_project_members($projectID);

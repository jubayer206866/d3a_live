...
class Api_model extends App_Model
{
    ...
    public function _search_tickets($q, $limit = 0, $api = false)
    {
        ...
        if (is_staff_member() || (!is_staff_member() && 1 == get_option('access_tickets_to_none_staff_members')) || true == $api) {
            ...
            $this->db->where('tbltickets.status !=', 5);
    ...
    public function add_user($data)
    {
        ...
        if ($insert_id) {
            log_activity('New User Added [ID: '.$insert_id.', Name: '.$data['name'].']','','تمت إضافة مستخدم جديد [ID: '.$insert_id.', Name: '.$data['name'].']');
        }
    ...
    public function update_user($data, $id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('Ticket User Updated [ID: '.$id.' Name: '.$data['name'].']','','تم تحديث مستخدم التذكرة [ID: '.$id.' Name: '.$data['name'].']');
            $result = true;
        }
    ...
    public function delete_user($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            log_activity('User Deleted [ID: '.$id.']','','تم حذف المستخدم [ID: '.$id.']');
    ...
    public function add_permissions($id = '', $feature = '', $capability = '')
    {
        ...
        foreach ($permissions as $permission) {
            ...
            if ($this->db->affected_rows() > 0) {
                log_activity('New API Permssion Added [API ID: ' . $permission['api_id'] . ', Feature: ' . $permission['feature'] . ', Capability: ' . $permission['capability'] . ']','','تمت إضافة إذن API جديد [API ID: ' . $permission['api_id'] . ', Feature: ' . $permission['feature'] . ', Capability: ' . $permission['capability'] . ']');
            }
        }
    }
    ...
    public function remove_permissions($id = '', $feature = '', $capability = '')
    {
        if ('' != $id) {
            ...
            if ($this->db->affected_rows() > 0) {
                log_activity('API Permssion Deleted [API ID: ' . $id . ', Feature: ' . $feature . ', Capability: ' . $capability . ']','','تم حذف إذن واجهة برمجة التطبيقات [API ID: ' . $id . ', Feature: ' . $feature . ', Capability: ' . $capability . ']');
    ...
    public function selectgoods($table, array $where = [])
    {
        $builder = $this->db->from($table);
        if (!empty($where)) {
            $builder->where($where);
        }

        $query = $builder->get();
        return $query->result(); // or ->result_array() if you want array
    }
    public function select($table, $where = [])
    {
        return $this->db->get_where($table, $where)->row_array(); // row() if object needed
    }
    
    public function updatevalue($table, $data,$where)
{
$this->db->where($where);
...
public function updatedelivery($table, $data,$where)
{
$this->db->where($where);
$updateResult = $this->db->update($table, $data);

    ...
    "use strict";
     ...
     var GoodsreceiptParams = {
        "customer": "select[name='filter_customer[]']",
        "project": "select[name='filter_project[]']",
        "supplier": "select[name='filter_supplier[]']",
        "buyer": "select[name='filter_buyer[]']",
    };
...
$('#filter_customer').on('change', function() {
    ...
    table_manage_goods_receipt.DataTable().ajax.reload();
});$('#filter_buyer').on('change', function() {
    table_manage_goods_receipt.DataTable().ajax.reload();
});
    ...
    if(confirm('Are you sure you want to convert this?')) {
        $.post(admin_url + 'purchase/change_delivery_status/' + status + '/' + id).done(function(response){
            ...
            if(response.success){
                ...
                alert_float('danger', response.mess);
            }
        });
    } else {
        return false;

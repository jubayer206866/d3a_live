...
/**
 * Serialize goods receipt line rows to JSON so saves work when PHP max_input_vars truncates large forms.
 */
function wh_collect_goods_receipt_line_items_for_json() {
	"use strict";
	var items = {};
	var newitems = {};
	var nameRe = /^(items|newitems)\[(\d+)\]\[([^\[\]]+)\](\[\])?$/;

	$('#add_goods_receipt .invoice-items-table tbody tr.item').each(function () {
		$(this).find('input, select, textarea').each(function () {
			...
			var idx = m[2];
			var field = m[3];
			var isArrayField = !!m[4];

			var bucket = bucketName === 'items' ? items : newitems;
			if (!bucket[idx]) {
				bucket[idx] = {};
			}
...
function submit_form(save_and_send_request) {
	...
	if(check_warehouse_status == true){
		var $form = $('#add_goods_receipt');
		var lineItemsPayload = wh_collect_goods_receipt_line_items_for_json();
		if (lineItemsPayload.items || lineItemsPayload.newitems) {
			var jsonStr = JSON.stringify(lineItemsPayload);
			var $ta = $form.find('textarea[name="goods_receipt_line_items_json"]');
			if (!$ta.length) {
				$ta = $('<textarea/>', { name: 'goods_receipt_line_items_json' }).css({ position: 'absolute', left: '-9999px', width: '1px', height: '1px' });
				$form.append($ta);
			}
			$ta.val(jsonStr);
			$form.find('.invoice-items-table tbody tr.item').find('[name]').removeAttr('name');
		}

...
$aColumns = [
    'tblgoods_delivery.id as id',
    'goods_delivery_code',
    'goods_receipt_code',
    'customer_code',
    'tblgoods_delivery.date_add as date_add',
    'invoice_id',
    'to_', 
    'address',
    'staff_id',
    'tblgoods_delivery.approval as approval',
    'delivery_status',
    ];
$sIndexColumn = 'tblgoods_delivery.id as id';
...
$join = [
    'LEFT JOIN ' . db_prefix() . 'goods_receipt ON ' . db_prefix() . 'goods_receipt.id = ' . db_prefix() . 'goods_delivery.goods_receipt_id',
...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, ['tblgoods_delivery.id','tblgoods_delivery.date_add','tblgoods_delivery.date_c','goods_delivery_code','tblgoods_delivery.total_money', 'type_of_delivery','goods_receipt_code']);
...
foreach ($rResult as $aRow) {
   ...
   for ($i = 0; $i < count($aColumns); $i++) {
    $CI           = & get_instance();
        ...
        if($aColumns[$i] == 'customer_code'){
        ...
        }elseif($aColumns[$i] == 'tblgoods_delivery.date_add as date_add'){
            ...
            $_data = $aRow['address'];
        }elseif($aColumns[$i] == 'tblgoods_delivery.approval as approval') {

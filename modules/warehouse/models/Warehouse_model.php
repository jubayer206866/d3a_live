...
class Warehouse_model extends App_Model
{
					...
					if ($from_po == true && $flag_update_status_po == true) {
						if (get_status_modules_wh('purchase')) {
							if ($this->db->field_exists('delivery_status', db_prefix() . 'pur_orders')) {
								...
								$this->db->update(db_prefix() . 'pur_orders', ['status_goods' => 1, 'delivery_status' => 0]);
							}
						}
	...
	public function get_stock_import_pdf_html($goods_receipt_id)
	{
		...
		$info_right_column .= '<b style="font-size:20px; text-align:center">Inventory Receiving</b>';

		$html = '';
		$html .= '<table class="table">
		<tbody>
        <tr>
        <td align="center">' . pdf_logo_url_custom() . '</td>
    </tr>
        <tr>
        	<td rowspan="1" width="50%" class="text-center">' . $info_right_column . '</td>
        </tr>
		</tbody>
		</table>
		<br><br>
		...
		//get vendor infor
		$invoice_date = '<b>' . _l('invoice_data_date') . ' ' . _d($goods_receipt->date_add) . '</b>';
		$bill_to = '<b>#' . $goods_receipt->goods_receipt_code . '<br></b>';
		$bill_to .= $invoice_date . '<br>';
		$customer_name = '';
		...
		$bill_to .= '<b>' . _l('customer') . '</b>';
		$bill_to .= '<div class="bill_to_color">';
		$bill_to .= get_customer_name_with_project($goods_receipt->project);
		$bill_to .= '</div>';
		...
		<tr>
            <th class="thead-dark" style="width: 11%;">Product Code</th>
            <th class="thead-dark" style="width: 12%;">Product Name</th>
            <th class="thead-dark" style="width: 7%;">Cartons</th>
            <th class="thead-dark" style="width: 7%;">Pieces Carton</th>
            <th class="thead-dark" style="width: 7%;">Total Pieces</th>
            <th class="thead-dark" style="width: 7%;">Price</th>
            <th class="thead-dark" style="width: 7%;">Price Total</th>
            <th class="thead-dark" style="width: 7%;">Net Weight</th>
            <th class="thead-dark" style="width: 7%;">Total Net Weight</th>
            <th class="thead-dark" style="width: 7%;">Gross Weight</th>
            <th class="thead-dark" style="width: 7%;">Total Gross Weight</th>
            <th class="thead-dark" style="width: 7%;">CBM</th>
            <th class="thead-dark" style="width: 7%;">Total CBM</th>
		...
		foreach ($goods_receipt_detail as $receipt_key => $receipt_value) {
			if ($receipt_value['checkd_koli'] != 0) {
				$vew_price_total = (strpos($receipt_value['price_total'], '¥') !== false) ? $receipt_value['price_total'] : '¥' . $receipt_value['price_total'];
				$html .= '<tr nobr="true" class="sortable">
            <td style="width: 11%;">' . $receipt_value['commodity_name'] . '</td>
            <td style="width: 12%;">' . $receipt_value['commodity_long_description'] . '</td>
            <td style="width: 7%;">' . $receipt_value['checkd_koli'] . '</td>
            <td style="width: 7%;">' . $receipt_value['cope_koli'] . '</td>
            <td style="width: 7%;">' . $receipt_value['total_koli'] . '</td>
            <td style="width: 7%;">' . $receipt_value['price'] . '</td>
            <td style="width: 7%;">' . $vew_price_total . '</td>
            <td style="width: 7%;">' . clean_number($receipt_value['net_weight']) . '</td>
            <td style="width: 7%;">' . clean_number($receipt_value['total_net_weight']) . '</td>
            <td style="width: 7%;">' . clean_number($receipt_value['gross_weight']) . '</td>
            <td style="width: 7%;">' . clean_number($receipt_value['total_gross_weight']) . '</td>
            <td style="width: 7%;">' . clean_number($receipt_value['cbm_koli']) . '</td>
            <td style="width: 7%;">' . clean_number($receipt_value['total_cbm']) . '</td>';
				$html .= '</tr>';
				$koli += $receipt_value['checkd_koli'];
				$total_koli += $receipt_value['total_koli'];
				$price_total += (float) preg_replace('/[^0-9.]/', '', $receipt_value['price_total']);
				$total_net_weight += $receipt_value['total_net_weight'];
				$total_gross_weight += $receipt_value['total_gross_weight'];
				$total_cbm += $receipt_value['total_cbm'];
			}
		}
		$html .= '</tbody>
		</table>
		...
		<td ></td>
		<td ></td>
		<td colspan="2" class="text_left"><b>Cartons </b></td>
		<td class="text_right"> ' . $koli . '</td>
		</tr>
		...
		<td ></td>
		<td ></td>
		<td colspan="2" class="text_left"><b>Total Pieces</b></td>
		<td class="text_right"> ' . $total_koli . '</td>
		</tr>
		...
		<td ></td>
		<td ></td>
		<td colspan="2" class="text_left"><b>Total Price</b></td>
		<td class="text_right"> ¥' . $price_total . '</td>
		</tr>
		<tr>
		<td ></td>
		<td ></td>
		<td ></td>
		<td colspan="2" class="text_left"><b>Total Gross Weight</b></td>
		<td class="text_right">    ' . clean_number($total_gross_weight) . '</td>
		</tr>
		...
		<td ></td>
				<td ></td>
		<td colspan="2" class="text_left"><b>Total Net Weight</b></td>
		<td class="text_right">  ' . clean_number($total_net_weight) . '</td>
		</tr>
		...
		<td ></td>
		<td ></td>
		<td colspan="2" class="text_left"><b>Total CBMss</b></td>
		<td class="text_right">  ' . clean_number($total_cbm) . '</td>
		</tr>';
		$html .= '</tbody>
		</table>
		<br><br><br>
	...
	public function print_packing_list_qrcode($project_id)
	{
		foreach ($pur_orders as $pur_order) {
			$vendor = array();
			$vendor['name'] = $pur_order['company'];
			$vendor['style'] = 'font-size:14px; font-weight:600;text-align:center;  background-color:#9ebbf0;';
			$vendor['span'] = 'colspan="14"';
			$return[][] = $vendor;
			$this->db->where('pur_order', $pur_order['id']);
			$this->db->where('pur_order', $pur_order['id']);
			$this->db->join(db_prefix() . 'items', db_prefix() . 'items.id=' . db_prefix() . 'pur_order_detail.item_code');
			$pur_order_items = $this->db->get(db_prefix() . 'pur_order_detail')->result_array();
			...
			$vendor_total_cbm = 0;
			foreach ($pur_order_items as $pur_order_item) {
				$items = array();
				$items[]['name'] = $pur_order_item['description'];
				if (isset($arr_images[$pur_order_item['id']]) && isset($arr_images[$pur_order_item['id']][0])) {
					$items[]['name'] = '<img class="images_w_table" src="' . site_url('modules/purchase/uploads/item_img/' . $arr_images[$pur_order_item['id']][0]['rel_id'] . '/' . $arr_images[$pur_order_item['id']][0]['file_name']) . '" alt="' . $arr_images[$pur_order_item['id']][0]['file_name'] . '" >';
				} else {
					$items[]['name'] = '';
				}
				$items[]['name'] = $pur_order_item['long_description'];
				$items[]['name'] = $pur_order_item['koli'];
				$vendor_koli += (float) $pur_order_item['koli'];
				$items[]['name'] = $pur_order_item['cope_koli'];
				$items[]['name'] = $pur_order_item['total_koli'];
				$vendor_total_peaces += (float) $pur_order_item['total_koli'];
				$items[]['name'] = $pur_order_item['rate'];
				$items[]['name'] = $pur_order_item['price_total'];
				$vendor_total_prices += (float) $pur_order_item['price_total'];
				$items[]['name'] = $pur_order_item['net_weight'];
				$items[]['name'] = $pur_order_item['total_net_weight'];
				$vendor_total_net_weight += (float) $pur_order_item['total_net_weight'];
				$items[]['name'] = $pur_order_item['gross_weight'];
				$items[]['name'] = $pur_order_item['total_gross_weight'];
				$vendor_total_gross_weight += (float) $pur_order_item['total_gross_weight'];
				$items[]['name'] = $pur_order_item['cbm_koli'];
				$items[]['name'] = $pur_order_item['total_cbm'];
				$vendor_total_cbm += (float) $pur_order_item['total_cbm'];
				$return[] = $items;
			}
			$sum = array();
			$sum_head['name'] = 'Total Of the ' . $pur_order['company'];
			$sum_head['style'] = 'font-size:14px; font-weight:600;text-align:center; background-color:#B8860B;';
			$sum_head['span'] = 'colspan="3"';
			$sum[] = $sum_head;
			$sum[]['name'] = $vendor_koli;
			$summary[0]['data'] += $vendor_koli;
			$sum[]['name'] = '';
			$sum[]['name'] = $vendor_total_peaces;
			$summary[1]['data'] += $vendor_total_peaces;
			$sum[]['name'] = '';
			$sum[]['name'] = $vendor_total_prices;
			$summary[2]['data'] += $vendor_total_prices;
			$sum[]['name'] = '';
			$sum[]['name'] = $vendor_total_net_weight;
			$summary[3]['data'] += $vendor_total_net_weight;
			$sum[]['name'] = '';
			$sum[]['name'] = $vendor_total_gross_weight;
			$summary[4]['data'] += $vendor_total_gross_weight;
			$sum[]['name'] = '';
			$sum[]['name'] = $vendor_total_cbm;
			$summary[5]['data'] += $vendor_total_cbm;

			foreach ($sum as $sum_key => $s) {
				if ($sum_key) {
					$sum[$sum_key]['style'] = 'background-color:#B8860B;';
				}
			}
			$return[] = $sum;
		}
	...
	public function add_goods_delivery($data, $id = false)
	{
		...
		/*update save note*/
		if (isset($insert_id)) {
			$goods_receipt_update['goods_delivery_id'] = $insert_id;
			$this->db->where('pr_order_id', $data['pr_order_id']);
			$this->db->update(db_prefix() . 'goods_receipt', $goods_receipt_update);
	...
	public function get_stock_export_pdf_html($id)
	{
		...
		$data['title'] = _l('New PDF');

		// Preload item images
		$item_images = [];
		foreach ($data['goods_receipt_detail'] as $item) {
			$attachments = $this->purchase_model->get_item_attachments($item['commodity_code']);
			if (!empty($attachments)) {
				$item_images[$item['commodity_code']] = $attachments;
			}
		}

		// Deposit
		$deposit = 0;
		if (isset($data['goods_receipt']->pr_order_id) && $data['goods_receipt']->pr_order_id > 0) {
			$this->db->select('deposit');
			$this->db->where('id', $data['goods_receipt']->pr_order_id);
			$query = $this->db->get(db_prefix() . 'pur_orders');
			if ($query->num_rows() > 0) {
				$deposit = (float)$query->row()->deposit;
			}
		}

		// Vendor Name
		$vendorName = $data['goods_receipt']->supplier_name;
		if (isset($data['goods_receipt']->supplier_code) && $data['goods_receipt']->supplier_code != 0) {
			$supplier = $this->purchase_model->get_vendor($data['goods_receipt']->supplier_code);
			if ($supplier) $vendorName = $supplier->company;
		}

		// Customer Name from pur_orders -> clients
		$customer_name = '';
		if (!empty($data['goods_receipt']->pr_order_id)) {
			$this->db->select('c.company');
			$this->db->from('pur_orders po');
			$this->db->join('clients c', 'c.userid = po.clients', 'left');
			$this->db->where('po.id', $data['goods_receipt']->pr_order_id);
			$client_row = $this->db->get()->row();

			if (!empty($client_row)) {
				$customer_name = $client_row->company;
			}
		}

		// Totals
		$total_cartons = $total_pieces = $total_price = $total_cbm = 0;
		foreach ($data['goods_receipt_detail'] as $item) {
			$total_cartons += $item['checkd_koli'];
			$total_pieces += $item['total_koli'];
			$total_price += (float) preg_replace('/[^0-9.]/', '', $item['price_total']);
		...
		$html = '<html><head>
					<meta charset="utf-8">
					<style>
						body { font-family: DejaVu Sans, sans-serif; font-size: 12px; color: #333; line-height: 1.2; }
		...
		if (file_exists($logo_path)) {
			...
			$logo_html = '<img src="' . $logo_base64 . '" width="200" style="display:block; margin:0 auto 2px auto;">';
		} else {
			$logo_html = '';
		}

		$company_name = isset($data['company_name']) ? $data['company_name'] : 'Company Name';
		$html .= '<div class="company-header" style="text-align:center; padding:0; line-height:1;">
					' . $logo_html . '
					<div style="font-size:20px; font-weight:bold; margin:0; padding:0;">' . $company_name . '</div>
					<div style="font-size:17px; font-weight:bold; margin:0; padding:0;">Inventory Receiving</div>
				</div>';

		// Header Information
		$html .= '<table width="100%" class="header-info">
					<tr>
						<td width="50%" style="text-align:left; vertical-align:top; padding:2px;">
							' . format_organization_info() . '
						</td>
						<td width="30%"></td>
						<td width="50%" style="text-align:right; vertical-align:top; padding:2px;">
							<strong>Invoice #:</strong> ' . $data['goods_receipt']->goods_receipt_code . '<br>
							<strong>Invoice Date:</strong> ' . _d($data['goods_receipt']->date_add) . '<br>
		...
		// Items Table
		$html .= '<br><br><table class="items-table">
					<thead>
						<tr style="font-weight:bold;">
							<th width="8%"><b>Image</b></th>
							<th width="12%"><b>Product Code</b></th>
							<th width="15%"><b>Product Name</b></th>
							<th width="8%"><b>Cartons</b></th>
							<th width="10%"><b>Pieces/Carton</b></th>
							<th width="10%"><b>Total Pieces</b></th>
							<th width="8%"><b>Price</b></th>
							<th width="12%"><b>Total Price</b></th>
							<th width="7%"><b>CBM</b></th>
							<th width="10%"><b>Total CBM</b></th>
						</tr>
					</thead>
					<tbody>';
		...
		foreach ($data['goods_receipt_detail'] as $item) {
			...
			if (empty($item['checkd_koli']) || $item['checkd_koli'] == 0) {
				continue;
			}

			$commodity_id = $item['commodity_code'];
			$img_html = '';

			if (isset($item_images[$commodity_id]) && count($item_images[$commodity_id]) > 0) {
				$img = $item_images[$commodity_id][0];
				$img_path = FCPATH . 'modules/purchase/uploads/item_img/' . $commodity_id . '/' . $img['file_name'];

				if (file_exists($img_path)) {
					$img_html = '<img src="' . $img_path . '" width="50px"/>';
				}
			}
			...
			$html .= '<tr>
				...
				<td class="text-left">' . $item['commodity_name'] . '</td>
				<td>' . $item['checkd_koli'] . '</td>
				<td>' . $item['cope_koli'] . '</td>
				<td>' . $item['total_koli'] . '</td>
				<td>¥' . number_format($item['price'], 2) . '</td>
				<td>¥' . number_format(preg_replace("/[^0-9.]/", "", $item['price_total']), 2) . '</td>
				<td>' . number_format($item['cbm_koli'], 3) . '</td>
		...
		$html .= '</tbody>
			<tfoot>
				<tr>
					<td colspan="3" class="text-right"><strong>Total of the </strong>' . $vendorName . '</td>
					<td>' . $total_cartons . '</td>
					<td></td>
					...
					<td></td>
					<td><strong>' . number_format($total_cbm, 3) . '</strong></td>
				</tr>
			</tfoot>
		</table>';
			...
			<tr>
				...
				<td width="30%">
					<table class="summary-table" style="width: 100%;">
						<tr>
							<td class="label" style="font-weight: bold;">Total Cost</td>
							<td>¥' . number_format($total_price, 2) . '</td>
						</tr>
						<tr>
							<td class="label" style="font-weight: bold;">Deposit</td>
							<td>¥' . number_format($deposit, 2) . '</td>
						</tr>
						<tr>
							<td class="label" style="font-weight: bold;">Remaining Balance</td>
							<td style="font-weight: bold;">¥' . number_format($remaining_balance, 2) . '</td>
						</tr>
					</table>
				</td>
			</tr>
	...
	public function import_xlsx_commodity($data, $flag_insert_id)
	{
		...
		$existing_item = $this->db->query('SELECT * FROM tblitems WHERE commodity_code = "' . $data['commodity_code'] . '"')->row();

		// Prepare SKU
		if ($data['sku_code'] != '' && strlen($data['sku_code']) > 0) {
			$data['sku_code'] = new_str_replace(' ', '', $data['sku_code']);
		} else {
			$data['sku_code'] = $this->create_sku_code($data['group_id'], $data['sub_group'] ?? '', 0);
		}

		// Append random number only for duplicates
		if ($existing_item) {
			$data['sku_code'] .= '-' . rand(100, 999);
		}

		$data['sku_code'] = get_warehouse_option('item_sku_prefix') . $data['sku_code'];

		// Extract minimum inventory and tags
		$minimum_inventory = $data['minimum_inventory'] ?? 0;
		...
		if ($insert_id) {
			...
			$data_inventory_min = [
				...
				'inventory_number_min' => $minimum_inventory,
			];
			$this->add_inventory_min($data_inventory_min);

			return ['status' => true, 'message' => 'Inserted successfully', 'insert_id' => $insert_id];
		}

		return ['status' => false, 'message' => 'Add item failed'];
	}
	...
	public function generate_commodity_barcode()
	{
		...
		do {
			...
			for ($i = 0; $i < $length; $i++) {
				...
				$baseDigits .= $chars[$index];
			}

			// Generate full 13-digit EAN with checksum
			$barcode = $this->generateEAN13Checksum($baseDigits);
	...
	function generateEAN13Checksum($digits)
	{
		if (!preg_match('/^\d{12}$/', $digits)) {
			return false; // Input must be 12 digits
		}
	...
	public function update_goods_receipt($data, $id = false)
	{
		...
		$this->db->where('goods_receipt_id', $data['id']);
		$pur_invoice = $this->db->get(db_prefix() . 'pur_invoices')->result_array();
		if (count($pur_invoice)) {
			$this->db->where('pur_invoice', $pur_invoice[0]['id']);
			$invoice_items = $this->db->get(db_prefix() . 'pur_invoice_details')->result_array();
			$invoice_item_ids = array();
			foreach ($invoice_items as $invoice_item) {
				$invoice_item_ids[$invoice_item['item_code']] = $invoice_item['id'];
			}
		}
		$this->db->where('goods_receipt_id', $data['id']);
		$goods_delivery = $this->db->get(db_prefix() . 'goods_delivery')->result_array();

		if (count($goods_delivery)) {
			$this->db->where('goods_delivery_id', $goods_delivery[0]['id']);
			...
			foreach ($goods_delivery_details as $goods_delivery_detail) {
				$delivery_item_ids[$goods_delivery_detail['commodity_code']] = $goods_delivery_detail['id'];
			}


			$this->db->where('delivery_note_id', $goods_delivery[0]['id']);
			$packing_lists = $this->db->get(db_prefix() . 'wh_packing_lists')->result_array();
			if (count($packing_lists)) {
				$this->db->where('packing_list_id', $packing_lists[0]['id']);
				$packing_list_details = $this->db->get(db_prefix() . 'wh_packing_list_details')->result_array();
				$list_item_ids = array();
				foreach ($packing_list_details as $packing_list_detail) {
					$list_item_ids[$packing_list_detail['commodity_code']] = $packing_list_detail['id'];
				}
			}
		}
		$inventory_receipts = [];
		...
		if (isset($data['items'])) {
			foreach ($data['items'] as $key => $items) {
				$update_inventory_receipts_single = array();
				//good receipt
				$update_inventory_receipts_single['id'] = $items['id'];
				$update_inventory_receipts_single['checkd_koli'] = $items['koli'];
				$update_inventory_receipts_single['cope_koli'] = $items['cope_koli'];
				$update_inventory_receipts_single['total_koli'] = (float) $items['koli'] * (float) $items['cope_koli'];
				$update_inventory_receipts_single['price'] = $items['price'];
			    $price_total = (float) preg_replace('/[^0-9.]/', '', $update_inventory_receipts_single['price']) * (float) $update_inventory_receipts_single['total_koli'];
				$update_inventory_receipts_single['price_total'] = '¥' . $price_total;
				$update_inventory_receipts_single['gross_weight'] = $items['gross_weight'];
				$update_inventory_receipts_single['total_gross_weight'] = (float) $items['gross_weight'] * (float) $update_inventory_receipts_single['checkd_koli'];
				$update_inventory_receipts_single['net_weight'] = $items['net_weight'];
				$update_inventory_receipts_single['total_net_weight'] = (float) $items['net_weight'] * (float) $update_inventory_receipts_single['checkd_koli'];
				$update_inventory_receipts_single['cbm_koli'] = $items['cbm_koli'];
				$update_inventory_receipts_single['total_cbm'] = (float) $items['cbm_koli'] * (float) $update_inventory_receipts_single['checkd_koli'];
				$update_inventory_receipts[] = $update_inventory_receipts_single;
				$amount += $price_total;


				//good rceipt end
				//purchase invoice start
				$update_inventory_receipts_single['id'] = $invoice_item_ids[$items['commodity_code']];
				$update_inventory_receipts_single['koli'] = $update_inventory_receipts_single['checkd_koli'];
				unset($update_inventory_receipts_single['checkd_koli']);
				$update_purchase_invoice[] = $update_inventory_receipts_single;
				//purchase invoice end

				//good delivery start
				$update_goods_delivery_single = array();
				$update_goods_delivery_single['id'] = $delivery_item_ids[$items['commodity_code']];
				$update_goods_delivery_single['koli'] = $items['receive_koli'];
				$update_goods_delivery_single['cope_koli'] = $items['cope_koli'];
				$update_goods_delivery_single['total_koli'] = (float) $items['receive_koli'] * (float) $items['cope_koli'];
				$update_goods_delivery_single['price'] = $items['price'];
				$price_total = (float) preg_replace('/[^0-9.]/', '', $update_goods_delivery_single['price']) * (float) $update_goods_delivery_single['total_koli'];
				$update_goods_delivery_single['price_total'] = '¥' . $price_total;
				$update_goods_delivery_single['gross_weight'] = $items['gross_weight'];
				$update_goods_delivery_single['total_gross_weight'] = (float) $items['gross_weight'] * (float) $items['receive_koli'];
				$update_goods_delivery_single['net_weight'] = $items['net_weight'];
				$update_goods_delivery_single['total_net_weight'] = (float) $items['net_weight'] * (float) $items['receive_koli'];
				$update_goods_delivery_single['cbm_koli'] = $items['cbm_koli'];
				$update_goods_delivery_single['total_cbm'] = (float) $items['cbm_koli'] * (float) $items['receive_koli'];
				$update_delivery_items[] = $update_goods_delivery_single;
				$delivery_amount += $price_total;
				//good delivery end
				//packing list start
				$update_goods_delivery_single['id'] = $list_item_ids[$items['commodity_code']];
				$update_list_items[] = $update_goods_delivery_single;
				//packing list end
			}
			unset($data['items']);
		}
		if (count($pur_invoice)) {
			//update pur invoice
			$this->db->where('id', $pur_invoice[0]['id']);
			$this->db->update(db_prefix() . 'pur_invoices', [
				'subtotal' => $amount,
			]);
			//pur invpice end

			//update pur invoice details 
			if (count($update_purchase_invoice)) {
				$this->db->update_batch('tblpur_invoice_details', $update_purchase_invoice, 'id');
			}
			//pur invoice details end
		}
		//update goods delivery
		$this->db->where('id', $goods_delivery[0]['id']);
		$this->db->update(db_prefix() . 'goods_delivery', [
			'total_money' => $delivery_amount,
			'sub_total' => $delivery_amount,
		]);
		if (count($update_delivery_items)) {
			$this->db->update_batch('tblgoods_delivery_detail', $update_delivery_items, 'id');
		}
		//update goods delivery end

		//update packing list
		$this->db->where('id', $packing_lists[0]['id']);
		$this->db->update(db_prefix() . 'wh_packing_lists', [
			'subtotal' => $delivery_amount,
			'total_amount' => $delivery_amount,
			'total_after_discount' => $delivery_amount,
		]);
		if (count($update_list_items)) {
			$this->db->update_batch('tblwh_packing_list_details', $update_list_items, 'id');
		}
		//update packing list end
		...
		$data['addedfrom'] = get_staff_user_id();
		$data['total_tax_money'] = reformat_currency_j($data['total_tax_money']);
		$data['total_goods_money'] = $amount;
		$data['value_of_inventory'] = $amount;
		...
		$goods_receipt = $this->db->get(db_prefix() . 'goods_receipt')->row();
 		...
 		$this->db->where('id', $goods_receipt->pr_order_id);
        $this->db->update(db_prefix() . 'pur_orders', ['project' => $data['project']]);

		/*update save note*/
		// update receipt note
		if (count($update_inventory_receipts)) {
			$this->db->update_batch('tblgoods_receipt_detail', $update_inventory_receipts, 'id');
		}
	...
	public function auto_create_goods_receipt_with_purchase_order($data)
	{
		...
		$sql = 'select tblitems.description as commodity_name, long_description as commodity_long_description,tblitems.koli,tblitems.cope_koli,tblitems.total_koli,tblitems.price_total,tblitems.gross_weight,tblitems.total_gross_weight,tblitems.net_weight,tblitems.total_net_weight,tblitems.cbm_koli,tblitems.total_cbm,tblitems.rate as price, item_code as commodity_code, ' . db_prefix() . 'items.description, ' . db_prefix() . 'items.unit_id, unit_price, quantity as quantities, ' . db_prefix() . 'pur_order_detail.tax as tax, into_money, (' . db_prefix() . 'pur_order_detail.total-' . db_prefix() . 'pur_order_detail.into_money) as tax_money, total as goods_money from ' . db_prefix() . 'pur_order_detail
    	left join ' . db_prefix() . 'items on ' . db_prefix() . 'pur_order_detail.item_code =  ' . db_prefix() . 'items.id
		...
		if ($get_pur_order) {
			if ($get_pur_order->currency_rate) {
				//$currency_rate = $get_pur_order->currency_rate;
			}
		}
		...
		if ($purchase_order) {
			...
			$arr_pur_resquest['supplier_code'] = $purchase_order->vendor;
			$arr_pur_resquest['project'] = $purchase_order->project;
		}
		...
		$id = $this->add_goods_receipt_from_purchase_order($arr_pur_resquest);
	...
	public function add_goods_receipt_from_purchase_order($data_insert)
	{
		...
		$data['supplier_code'] = $data_insert['supplier_code'];
		$data['project'] = $data_insert['project'];
		...
		if (isset($insert_id) && (count($data_insert['goods_receipt_detail']) > 0)) {
			...
			foreach ($data_insert['goods_receipt_detail'] as $purchase_key => $purchase_value) {
				...
				if (isset($purchase_value['item_code'])) {
					$purchase_value['commodity_code'] = $purchase_value['item_code'];
					unset($purchase_value['item_code']);
				}
				if (isset($purchase_value['item_name'])) {
					$purchase_value['commodity_name'] = $purchase_value['item_name'];
					unset($purchase_value['item_name']);
				}
				if (isset($purchase_value['item_description'])) {
					$purchase_value['commodity_long_description'] = $purchase_value['item_description'];
					unset($purchase_value['item_description']);
				}

				$purchase_value['warehouse_id'] = $warehouse_id;
				$purchase_value['goods_receipt_id'] = $insert_id;

				unset($purchase_value['order']);
				unset($purchase_value['id']);
				unset($purchase_value['tax_value']);
				unset($purchase_value['discount']);
				unset($purchase_value['discount_money']);
				unset($purchase_value['unit_id']);
				unset($purchase_value['total']);
				unset($purchase_value['total_money']);
				unset($purchase_value['quantity']);
				unset($purchase_value['total_cbm']);
				if (!isset($purchase_value['quantities'])) {
					$purchase_value['quantities'] = 1;
				}
	...
	public function goods_delivery_get_pur_order($pur_order)
	{
		...
		if (count($arr_results) > 0) {
			...
			foreach ($arr_results as $key => $value) {
				...
				$this->db->where('commodity_code', $value['commodity_code']);
				$this->db->where('goods_receipt_id', $goods_receipt[0]['id']);
				$receipt_detail = $this->db->get(db_prefix() . 'goods_receipt_detail')->result_array();
						...
						if ($temporaty_quantity > 0) {
							...
							foreach ($list_temporaty_serial_numbers as $value) {
								...
								if ($temporaty_available_quantity > 0) {
									...
									$goods_delivery_row_template .= $this->create_goods_delivery_row_template([], $name, $temporaty_commodity_name, $warehouse_id, $temporaty_available_quantity, $quantities, $unit_name, $unit_price, $taxname, $commodity_code, $unit_id, $tax_rate, '', '', '', 'total_after_discount', $guarantee_period, $expiry_date, $lot_number, $note, $sub_total, $tax_name, $tax_id, 'undefined', true, false, $value['serial_number'], $receipt_detail[0]['commodity_long_description'], 0, $receipt_detail[0]['cope_koli'], $receipt_detail[0]['total_koli'], $receipt_detail[0]['price'], $receipt_detail[0]['price_total'], $receipt_detail[0]['net_weight'], $receipt_detail[0]['total_net_weight'], $receipt_detail[0]['gross_weight'], $receipt_detail[0]['total_gross_weight'], $receipt_detail[0]['cbm_koli'], $receipt_detail[0]['total_cbm']);
									$temporaty_quantity--;
									$temporaty_available_quantity--;
									$item_index++;
									$inventory_warehouse_by_commodity[$key]['inventory_number'] = $temporaty_available_quantity;
					...
					if ($temporaty_quantity > 0) {
						...
						foreach ($inventory_warehouse_by_commodity as $key => $inventory_warehouse) {
							if ((float) $inventory_warehouse['inventory_number'] > 0 && $temporaty_quantity > 0) {
								...
								$goods_delivery_row_template .= $this->create_goods_delivery_row_template([], $name, $commodity_name, '', $available_quantity, $quantities, $unit_name, $unit_price, $taxname, $commodity_code, $unit_id, $tax_rate, '', '', '', $total_after_discount, $guarantee_period, $expiry_date, $lot_number, $note, $sub_total, $tax_name, $tax_id, 'undefined', true, '', '', '', $receipt_detail[0]['commodity_long_description'], 0, $receipt_detail[0]['cope_koli'], $receipt_detail[0]['total_koli'], $receipt_detail[0]['price'], $receipt_detail[0]['price_total'], $receipt_detail[0]['net_weight'], $receipt_detail[0]['total_net_weight'], $receipt_detail[0]['gross_weight'], $receipt_detail[0]['total_gross_weight'], $receipt_detail[0]['cbm_koli'], $receipt_detail[0]['total_cbm']);
								$item_index++;
							}
						}
					}
		...
		$arr_pur_resquest['userid'] = $get_pur_order->clients;
	...
	public function get_receipt_order_delivered($id = false)
	{
		if (!$id) {
			$this->db->where('goods_delivery_id IS NULL');
		}
	...
	{
		try {
			log_message('debug', 'Creating PDF object. HTML size: ' . number_format(strlen($print_barcode)) . ' bytes');
			$pdf = app_pdf('print_barcode', module_dir_path(WAREHOUSE_MODULE_NAME, 'libraries/pdf/Print_barcode_pdf.php'), $print_barcode);
			
			if (!$pdf) {
				log_message('error', 'app_pdf returned null');
				return false;
			}
			
			log_message('debug', 'PDF object created successfully');
			return $pdf;
		} catch (Exception $e) {
			log_message('error', 'Error in print_barcode_pdf: ' . $e->getMessage());
			log_message('error', 'File: ' . $e->getFile() . ' Line: ' . $e->getLine());
			throw $e;
		} catch (Error $e) {
			log_message('error', 'Fatal error in print_barcode_pdf: ' . $e->getMessage());
			log_message('error', 'File: ' . $e->getFile() . ' Line: ' . $e->getLine());
			throw $e;
		}
	}
	...
	public function po_barcode_pdf_html($ids, $shipping_mark)
	{
		...
		$html .= '<table class="table">
		<tbody>';
		$item_select_print_barcode_id = array_filter($ids);
		$sql_where = 'select * from ' . db_prefix() . 'items where id IN (' . implode(", ", $item_select_print_barcode_id) . ') order by id desc';
		$array_commodity = $this->db->query($sql_where)->result_array();
		...
		// Pre-generate all barcode images to avoid issues during HTML generation
		foreach ($array_commodity as $key => $value) {
			if ($value['commodity_barcode'] != '') {
				$barcode_file_full = FCPATH . WAREHOUSE_PRINT_ITEM . md5(trim($value['commodity_barcode'])) . '.svg';
				// Check if file exists and is valid
				$needs_generation = true;
				if (file_exists($barcode_file_full)) {
					$file_content = @file_get_contents($barcode_file_full);
					if ($file_content && strlen(trim($file_content)) > 200 && strpos($file_content, '<rect') !== false) {
						$needs_generation = false;
					} else {
						@unlink($barcode_file_full);
					}
				}
				if ($needs_generation) {
					$this->getBarcode(trim($value['commodity_barcode']));
				}
			}
		}
		...
		foreach ($array_commodity as $key => $value) {
			if (new_strlen($value['long_description']) > 30) {
				$sub = substr($value['long_description'], 0, 20);
			} else {
				$sub = $value['long_description'];
			}
			if (new_strlen($value['description']) > 30) {
			...
			$description = 'Emri: ' . $sub . '<br>Kodi: ' . $description . '<br>Vendi Origjines: P.R.C<br>Importuesi:' . $shipping_mark;
			...
			// Use absolute file path for TCPDF - it handles file paths better than base64 for large batches
			$barcode_file_path = FCPATH . 'modules/warehouse/uploads/print_item/' . md5(trim($value['commodity_barcode']) ?? '') . '.svg';
			$barcode_img_src = '';
			
			if ($value['commodity_barcode'] != '') {
				if (file_exists($barcode_file_path)) {
					// Use absolute file path - TCPDF can handle this better than base64 for large batches
					// Ensure path uses forward slashes (works on both Windows and Linux)
					$barcode_img_src = str_replace('\\', '/', $barcode_file_path);
				} else {
					// Use path anyway - TCPDF will handle missing file gracefully
					$barcode_img_src = str_replace('\\', '/', $barcode_file_path);
				}
			}
			...
			for ($i = 1; $i <= $print_qty; $i++) {
				...
				if ($value['commodity_barcode'] != '' && $barcode_img_src != '') {
					$html_child .= '<td style="page-break-inside: avoid; break-inside: avoid;"><span class="print-item-code print-item-name">' . $description . '</span><span class="print-item"><img class="images_w_table" src="' . $barcode_img_src . '" alt="' . trim($value['commodity_barcode']) . '" ></span><span class="print-item-code">' . trim($value['commodity_barcode']) . '</span></td>';

				} else {
					$html_child .= '<td style="page-break-inside: avoid; break-inside: avoid;"><span class="print-item-code print-item-name">' . $description . '</span><span class="print-item"><img class="images_w_table" src="" alt="' . trim($value['commodity_barcode']) . '" ></span><span class="print-item-code">' . _l('the_product_has_no_barcode') . '</span></td>';
				}

				if (($item_index + 1) % 4 == 0) {
					$html .= '<tr style="page-break-inside: avoid; break-inside: avoid;">' . $html_child . '</tr>';
		...
		// Ensure HTML is never null
		if ($html === null) {
			$html = '';
			log_message('error', 'HTML was null in po_barcode_pdf_html, setting to empty string');
		}
		
		// Log HTML size for debugging
		$html_size = strlen($html);
		log_message('debug', 'Regular HTML generated. Size: ' . number_format($html_size) . ' bytes');
		
		return $html;
	}

	/**
	 * po_barcode_pdf_html_batch - Optimized for large batches (100+ items)
	 * Processes items in chunks to avoid memory issues
	 * @param  array $ids 
	 * @param  string $shipping_mark 
	 * @return string HTML
	 */
	public function po_barcode_pdf_html_batch($ids, $shipping_mark)
	{
		$display_product_name = get_warehouse_option('display_product_name_when_print_barcode');
		$display_price = get_warehouse_option('wh_show_price_when_print_barcode');
		...
		// Process in very small batches (20 items) to minimize memory usage with 1000+ items
		// Smaller batches = less memory per batch, more frequent garbage collection
		$batch_size = 20;
		$total_items = count($item_select_print_barcode_id);
		$batches = array_chunk($item_select_print_barcode_id, $batch_size);
		...
		foreach ($batches as $batch_num => $batch_ids) {
			// Generate barcode images for this batch first
			$sql_where = 'select * from ' . db_prefix() . 'items where id IN (' . implode(", ", $batch_ids) . ') order by id desc';
			$array_commodity = $this->db->query($sql_where)->result_array();
			
			// Pre-generate barcode images for this batch
			foreach ($array_commodity as $value) {
				if ($value['commodity_barcode'] != '') {
					$barcode_file = WAREHOUSE_PRINT_ITEM . md5(trim($value['commodity_barcode'])) . '.svg';
			...
			foreach ($array_commodity as $key => $value) {
				...
				// Use absolute file path for TCPDF
				$barcode_file_path = FCPATH . 'modules/warehouse/uploads/print_item/' . md5(trim($value['commodity_barcode']) ?? '') . '.svg';
				$barcode_img_src = '';
				...
				for ($i = 1; $i <= $print_qty; $i++) {
					...
					if (($item_index + 1) % 4 == 0) {
						...
						$html_child = '';
						// Clear the variable to free memory immediately
						unset($html_child);
						$html_child = '';
					} elseif (($item_index + 1) % 4 != 0 && ($item_index + 1 == $total_items * $print_qty)) {
						$html .= '<tr style="page-break-inside: avoid; break-inside: avoid;">' . $html_child . '</tr>';
				...
				// Clear variables after each item to free memory
				unset($description, $sub, $tax_value, $rate_after_tax, $price_html, $barcode_file_path, $barcode_img_src);
			}
			
			// Clear batch data from memory immediately
			unset($array_commodity);
			$array_commodity = null;
			
			// Force garbage collection after every batch for very large sets
			if ($total_items > 500) {
				// For 500+ items, be very aggressive with memory management
				if (function_exists('gc_collect_cycles')) {
					gc_collect_cycles();
				}
			} else {
				// For smaller sets, collect every 2 batches
				if (($batch_num + 1) % 2 == 0) {
					if (function_exists('gc_collect_cycles')) {
						gc_collect_cycles();
					}
		...
		if ($html === null) {
			$html = '';
			log_message('error', 'HTML was null in po_barcode_pdf_html_batch, setting to empty string');
		}
		
		// Log HTML size for debugging
		$html_size = strlen($html);
		if ($html_size > 50000000) { // 50MB
			log_message('error', 'HTML size is very large: ' . number_format($html_size) . ' bytes. This may cause memory issues.');
		} else {
			log_message('debug', 'Batch HTML generated successfully. Size: ' . number_format($html_size) . ' bytes');
		}
	...
	function getBarcode($data)
	{
		...
		$barcode_file = WAREHOUSE_PRINT_ITEM . md5($data) . '.svg';
		$barcode_file_full = FCPATH . $barcode_file;
		
		// Delete existing file if it's empty or invalid
		if (file_exists($barcode_file_full)) {
			$existing_content = @file_get_contents($barcode_file_full);
			if ($existing_content && strlen(trim($existing_content)) > 200 && strpos($existing_content, '<rect') !== false) {
				// File exists and is valid, no need to regenerate
				return true;
			} else {
		...
		// Determine the best barcode type to use
		$barcodeType = $this->determineBestBarcodeType($data);
		
		// Generate barcode with determined type
		$result = $this->generateBarcodeWithType($data, $barcodeType, $barcode_file_full);
		
		// Verify the generated file is valid
		if ($result && file_exists($barcode_file_full)) {
			$generated_content = @file_get_contents($barcode_file_full);
			if (!$generated_content || strlen(trim($generated_content)) < 200 || strpos($generated_content, '<rect') === false) {
				// Generated file is invalid, try CODE128 as last resort
				@unlink($barcode_file_full);
				if ($barcodeType !== 'C128') {
					return $this->generateBarcodeWithType($data, 'C128', $barcode_file_full);
				}
				return false;
			}
		}
	...
	/**
	 * Determine the best barcode type for the given data
	 * @param  string $data 
	 * @return string barcode type
	 */
	private function determineBestBarcodeType($data)
	{
		// If not numeric, must use CODE128
		if (!ctype_digit($data)) {
			return 'C128';
		}
		...
		// For 13-digit codes, validate EAN-13 checksum
		if ($length == 13) {
			if ($this->validateEAN13Checksum($data)) {
				return 'EAN13';
			} else {
				// Invalid EAN-13, use CODE128
				return 'C128';
			}
		}
		
		// For 8-digit codes, try EAN-8 (but CODE128 is safer)
		if ($length == 8) {
			// EAN-8 also requires checksum validation, but CODE128 is more reliable
			return 'C128';
		}
		
		// For 12-digit codes, could be UPC-A, but CODE128 is safer
		if ($length == 12) {
			return 'C128';
		}
		
		// For any other length or format, use CODE128 (most flexible)
		return 'C128';
	}

	/**
	 * Generate barcode with specific type
	 * @param  string $data 
	 * @param  string $type 
	 * @param  string $file_path Full path to save the file
	 * @return bool
	 */
	private function generateBarcodeWithType($data, $type, $file_path = null)
	{
		if ($file_path === null) {
			$file_path = FCPATH . WAREHOUSE_PRINT_ITEM . md5($data) . '.svg';
		}
		...
		try {
			...
			// Validate barcode array BEFORE generating SVG
			$barcodeArray = $barcodeobj->getBarcodeArray();
			if (!$this->isValidBarcodeArray($barcodeArray)) {
				if ($type === 'C128') {
					log_message('error', 'CODE128 barcode validation failed for: ' . $data . ' - Array: ' . print_r($barcodeArray, true));
					return false;
				}
				// For other types, try CODE128 as fallback
				log_message('debug', 'Barcode validation failed for: ' . $data . ' with type: ' . $type . ', trying CODE128');
				return $this->generateBarcodeWithType($data, 'C128', $file_path);
			}
			...
			if (!$code || strlen(trim($code)) < 200) {
				if ($type === 'C128') {
					log_message('error', 'CODE128 generated empty or too short SVG for: ' . $data . ' - Length: ' . strlen($code));
					return false;
				}
				log_message('debug', 'Empty SVG generated for: ' . $data . ' with type: ' . $type . ', trying CODE128');
				return $this->generateBarcodeWithType($data, 'C128', $file_path);
			}
			
			// Check for actual barcode bars in SVG
			if (strpos($code, '<rect') === false) {
				if ($type === 'C128') {
					log_message('error', 'CODE128 SVG has no bars for: ' . $data);
					return false;
				}
				log_message('debug', 'SVG has no bars for: ' . $data . ' with type: ' . $type . ', trying CODE128');
				return $this->generateBarcodeWithType($data, 'C128', $file_path);
			}
			...
			if ($written === false) {
				log_message('error', 'Failed to write barcode file: ' . $file_path);
				return false;
			}
			
			// Verify file was written correctly
			if (!file_exists($file_path) || filesize($file_path) < 200) {
				log_message('error', 'Barcode file verification failed: ' . $file_path);
				@unlink($file_path);
				return false;
			}
			...
			if ($type === 'C128') {
				log_message('error', 'Barcode generation exception for: ' . $data . ' with type: ' . $type . ' - Error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
				return false;
			}
			log_message('debug', 'Exception for: ' . $data . ' with type: ' . $type . ', trying CODE128 - Error: ' . $e->getMessage());
			return $this->generateBarcodeWithType($data, 'C128', $file_path);
		} catch (Error $e) {
			if ($type === 'C128') {
				log_message('error', 'Barcode generation error for: ' . $data . ' with type: ' . $type . ' - Error: ' . $e->getMessage() . ' in ' . $e->getFile() . ':' . $e->getLine());
				return false;
			}
			log_message('debug', 'Error for: ' . $data . ' with type: ' . $type . ', trying CODE128 - Error: ' . $e->getMessage());
			return $this->generateBarcodeWithType($data, 'C128', $file_path);
		}
	}

	/**
	 * Validate barcode array structure
	 * @param  mixed $barcodeArray 
	 * @return bool
	 */
	private function isValidBarcodeArray($barcodeArray)
	{
		if ($barcodeArray === false || !is_array($barcodeArray)) {
			return false;
		}
		
		// Check required keys exist
		if (!isset($barcodeArray['maxw']) || !isset($barcodeArray['maxh']) || !isset($barcodeArray['bcode'])) {
			return false;
		}
		
		// Check maxw is valid (greater than 0)
		if ($barcodeArray['maxw'] <= 0) {
			return false;
		}
		
		// Check bcode is an array with content
		if (!is_array($barcodeArray['bcode']) || empty($barcodeArray['bcode'])) {
			return false;
		}
	 ...
	 * @param  string $code 13-digit code
	 * @return bool
	 */
	private function validateEAN13Checksum($code)
	{
	 ...
	 * @return [type]                    
	 */
	public function create_goods_receipt_row_template($warehouse_data = [], $name = '', $commodity_name = '', $warehouse_id = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '', $lot_number = '', $date_manufacture = '', $expiry_date = '', $commodity_code = '', $unit_id = '', $tax_rate = '', $tax_money = '', $goods_money = '', $note = '', $item_key = '', $sub_total = '', $tax_name = '', $tax_id = '', $is_edit = false, $serial_number = '', $commodity_long_description = '', $koli = '', $cope_koli = '', $total_koli = '', $rate = '', $price_total = '', $gross_weight = '', $total_gross_weight = '', $net_weight = '', $total_net_weight = '', $cbm_koli = '', $total_cbm = '', $receive_koli = '')
	{
		...
		$name_commodity_long_description = 'commodity_long_description';
		...
		$name_cartons = 'cartons';
		$name_receive_cartons = 'receive_cartons';
		$name_pieces_carton = 'pieces_carton';
		$name_total_pieces = 'total_pieces';
		$name_price = 'price';
		$name_Price_total = 'price_total';
		$name_net_weight = 'net_weight';
		$name_total_net_weight = 'total_net_weight';
		$name_gross_weight = 'gross_weight';
		$name_total_gross_weight = 'total_gross_weight';
		$name_cbm = 'cbm';
		$name_total_cbm = 'total_cbm';
		//custom
		...
		if ($name == '') {
			...
			$name_commodity_long_description = $name . '[commodity_long_description]';
			...
			//custom
			$name_cartons = $name . '[koli]';
			$name_receive_cartons = $name . '[receive_koli]';
			$name_pieces_carton = $name . '[cope_koli]';
			$name_total_pieces = $name . '[total_koli]';
			$name_price = $name . '[price]';
			$name_Price_total = $name . '[price_total]';
			$name_net_weight = $name . '[net_weight]';
			$name_total_net_weight = $name . '[total_net_weight]';
			$name_gross_weight = $name . '[gross_weight]';
			$name_total_gross_weight = $name . '[total_gross_weight]';
			$name_cbm = $name . '[cbm_koli]';
			$name_total_cbm = $name . '[total_cbm]';
			//custom
		...
		$row .= '<td class="warehouse_select hide">' .
			...
			render_select($name_warehouse_id, $warehouse_data, array('warehouse_id', 'warehouse_name'), '', 1, [], ["data-none-selected-text" => _l('warehouse_name')], 'no-margin') .
			render_input($name_note, '', $note, 'text', ['placeholder' => _l('commodity_notes')], [], 'no-margin', 'input-transparent text-left') .
			'</td>';
		$row .= '<td class="">' . render_textarea($name_commodity_name, '', $commodity_name, ['rows' => 2, 'placeholder' => _l('pur_item_name'), 'readonly' => true]) . '</td>';

		$row .= '<td class="">' . render_textarea($name_commodity_long_description, '', $commodity_long_description, ['rows' => 2, 'placeholder' => _l('item_description')]) . '</td>';

		$row .= '<td class="koli">' . render_input($name_cartons, '', $koli, 'text', ['placeholder' => 'Cartons']) . '</td>';
		$row .= '<td class="koli">' . render_input($name_receive_cartons, '', $receive_koli, 'text', ['placeholder' => 'Receive Cartons']) . '</td>';
		$row .= '<td class="cope_koli">' . render_input($name_pieces_carton, '', $cope_koli, 'text', ['placeholder' => 'Pieces/Carton']) . '</td>';
		$row .= '<td class="total_koli hide">' . render_input($name_total_pieces, '', $total_koli, 'text', ['placeholder' => 'Total/Pieces']) . '</td>';
		$row .= '<td class="Price">' . render_input($name_price, '', $rate, 'text', ['placeholder' => 'Price']) . '</td>';
		$row .= '<td class="price_total hide">' . render_input($name_Price_total, '', $price_total, 'text', ['placeholder' => 'Price/Total']) . '</td>';
		$row .= '<td class="gross_weight">' . render_input($name_gross_weight, '', $gross_weight, 'text', ['placeholder' => 'G.W']) . '</td>';
		$row .= '<td class="total_gross_weight hide">' . render_input($name_total_gross_weight, '', $total_gross_weight, 'text', ['placeholder' => 'T.G.W']) . '</td>';
		$row .= '<td class="net_weight">' . render_input($name_net_weight, '', $net_weight, 'text', ['placeholder' => 'N.W']) . '</td>';
		$row .= '<td class="total_net_weight hide">' . render_input($name_total_net_weight, '', $total_net_weight, 'text', ['placeholder' => 'T.N.W']) . '</td>';
		$row .= '<td class="cbm_koli">' . render_input($name_cbm, '', $cbm_koli, 'text', ['placeholder' => 'CBM']) . '</td>';
		$row .= '<td class="total_cbm hide">' . render_input($name_total_cbm, '', $total_cbm, 'text', ['placeholder' => 'Total CBM']) . '</td>';


		$row .= '<td class="quantities hide">' .
		...
		$row .= '<td class="rate hide">' . render_input($name_unit_price, '', $unit_price, 'number', $array_rate_attr) . '</td>';
		...
		$row .= '<td class="hide">' . render_input($name_lot_number, '', $lot_number, 'text', $lot_number_attr) . '</td>';

		$row .= '<td class="hide">' . render_date_input($name_date_manufacture, '', $date_manufacture, ['placeholder' => _l('date_manufacture')]) . '</td>';
		$row .= '<td class="hide">' . render_date_input($name_expiry_date, '', $expiry_date, ['placeholder' => _l('expiry_date')]) . '</td>';
	...
	public function create_goods_delivery_row_template($warehouse_data = [], $name = '', $commodity_name = '', $warehouse_id = '', $available_quantity = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '', $commodity_code = '', $unit_id = '', $tax_rate = '', $total_money = '', $discount = '', $discount_money = '', $total_after_discount = '', $guarantee_period = '', $expiry_date = '', $lot_number = '', $note = '', $sub_total = '', $tax_name = '', $tax_id = '', $item_key = '', $is_edit = false, $is_purchase_order = false, $serial_number = '', $without_checking_warehouse = 0, $commodity_long_description = '', $koli = '', $cope_koli = '', $total_koli = '', $rate = '', $price_total = '', $gross_weight = '', $total_gross_weight = '', $net_weight = '', $total_net_weight = '', $cbm_koli = '', $total_cbm = '', $id = '')
	{
		...
		$row .= '<td class="koli">' . render_input($name_cartons, '', $koli, 'text', ['placeholder' => 'Cartons']) . '</td>';
		$row .= '<td class="cope_koli">' . render_input($name_pieces_carton, '', $cope_koli, 'text', ['placeholder' => 'Pieces/Carton']) . '</td>';
		$row .= '<td class="total_koli">' . render_input($name_total_pieces, '', $total_koli, 'text', ['placeholder' => 'Total/Pieces']) . '</td>';
		$row .= '<td class="Price">' . render_input($name_price, '', $rate, 'text', ['placeholder' => 'Price']) . '</td>';
		$row .= '<td class="price_total">' . render_input($name_Price_total, '', $price_total, 'text', ['placeholder' => 'Price/Total']) . '</td>';
		$row .= '<td class="gross_weight">' . render_input($name_gross_weight, '', $gross_weight, 'text', ['placeholder' => 'G.W']) . '</td>';
		$row .= '<td class="total_gross_weight">' . render_input($name_total_gross_weight, '', $total_gross_weight, 'text', ['placeholder' => 'T.G.W']) . '</td>';
		$row .= '<td class="net_weight">' . render_input($name_net_weight, '', $net_weight, 'text', ['placeholder' => 'N.W']) . '</td>';
		$row .= '<td class="total_net_weight">' . render_input($name_total_net_weight, '', $total_net_weight, 'text', ['placeholder' => 'T.N.W']) . '</td>';
		$row .= '<td class="cbm_koli">' . render_input($name_cbm, '', $cbm_koli, 'text', ['placeholder' => 'CBM']) . '</td>';
		$row .= '<td class="total_cbm">' . render_input($name_total_cbm, '', $total_cbm, 'text', ['placeholder' => 'Total CBM']) . '</td>';
		...
		$row .= '<td class="warehouse_select hide">' .
			...
			render_input($name_note, '', $note, 'text', ['placeholder' => _l('commodity_notes')], [], 'no-margin', 'input-transparent text-left') .
			'</td>';
		$row .= '<td class="available_quantity hide">' .
			render_input($name_available_quantity, '', $available_quantity, 'number', $array_available_quantity_attr, [], 'no-margin') .
			render_input($name_unit_name, '', $unit_name, 'text', ['placeholder' => _l('unit'), 'readonly' => true], [], 'no-margin', 'input-transparent text-right wh_input_none') .
			'</td>';
		$row .= '<td class="quantities hide">' . render_input($name_quantities, '', $quantities, 'number', $array_qty_attr, [], 'no-margin') .
			...
			'</td>';
		$row .= '<td class="id hide">' . render_input($name_id, '', $id, 'number', []) . '</td>';
		$row .= '<td class="rate hide">' . render_input($name_unit_price, '', $unit_price, 'number', $array_rate_attr) . '</td>';
		$row .= '<td class="taxrate hide">' . $this->get_taxes_dropdown_template($name_tax_id_select, $invoice_item_taxes, 'invoice', $item_key, true, $manual) . '</td>';
		...
		$row .= '<td class="amount hide" align="right">' . $amount . '</td>';
		$row .= '<td class="discount hide">' . render_input($name_discount, '', $discount, 'number', $array_discount_attr) . '</td>';
		$row .= '<td class="label_discount_money hide" align="right">' . $amount . '</td>';
		$row .= '<td class="label_total_after_discount hide" align="right">' . $amount . '</td>';
	...
	public function create_packing_list_row_template($delivery_detail_id = '', $name = '', $commodity_name = '', $quantities = '', $unit_name = '', $unit_price = '', $taxname = '', $commodity_code = '', $unit_id = '', $tax_rate = '', $total_amount = '', $discount = '', $discount_total = '', $total_after_discount = '', $sub_total = '', $tax_name = '', $tax_id = '', $item_key = '', $is_edit = false, $max_qty = false, $serial_number = '', $commodity_long_description = '', $koli = '', $cope_koli = '', $total_koli = '', $rate = '', $price_total = '', $gross_weight = '', $total_gross_weight = '', $net_weight = '', $total_net_weight = '', $cbm_koli = '', $total_cbm = '')
	{
		...
		//custom
		$name_long_description = 'commodity_long_description';
		$name_cartons = 'koli';
		$name_pieces_carton = 'cope_koli';
		$name_total_pieces = 'total_koli';
		$name_price = 'price';
		$name_Price_total = 'price_total';
		$name_net_weight = 'net_weight';
		$name_total_net_weight = 'total_net_weight';
		...
		$row .= '<td class="">' . render_textarea($name_commodity_name, '', $commodity_name, ['rows' => 2, 'placeholder' => _l('item_description_placeholder'), 'readonly' => true]) . '</td>';
		$row .= '<td class="">' . render_textarea($name_commodity_long_description, '', $commodity_long_description, ['rows' => 2, 'placeholder' => _l('commodity_long_description'), 'readonly' => true]) . '</td>';
		$row .= '<td class="koli">' . render_input($name_cartons, '', $koli, 'text', ['placeholder' => 'Cartons']) . '</td>';
		$row .= '<td class="cope_koli">' . render_input($name_pieces_carton, '', $cope_koli, 'text', ['placeholder' => 'Pieces/Carton']) . '</td>';
		$row .= '<td class="total_koli">' . render_input($name_total_pieces, '', $total_koli, 'text', ['placeholder' => 'Total/Pieces']) . '</td>';
		$row .= '<td class="Price">' . render_input($name_price, '', $rate, 'text', ['placeholder' => 'Price']) . '</td>';
		$row .= '<td class="price_total">' . render_input($name_Price_total, '', $price_total, 'text', ['placeholder' => 'Price/Total']) . '</td>';
		$row .= '<td class="gross_weight">' . render_input($name_net_weight, '', $gross_weight, 'text', ['placeholder' => 'N.W']) . '</td>';
		$row .= '<td class="total_gross_weight">' . render_input($name_total_net_weight, '', $total_gross_weight, 'text', ['placeholder' => 'T.N.W']) . '</td>';
		$row .= '<td class="net_weight">' . render_input($name_gross_weight, '', $net_weight, 'text', ['placeholder' => 'G.W']) . '</td>';
		$row .= '<td class="total_net_weight">' . render_input($name_total_gross_weight, '', $total_net_weight, 'text', ['placeholder' => 'T.G.W']) . '</td>';
		$row .= '<td class="cbm_koli">' . render_input($name_cbm, '', $cbm_koli, 'text', ['placeholder' => 'CBM']) . '</td>';
		$row .= '<td class="total_cbm">' . render_input($name_total_cbm, '', $total_cbm, 'text', ['placeholder' => 'Total CBM']) . '</td>';


		$row .= '<td class="hide quantities">' .
			render_input($name_quantities, '', $quantities, 'number', $array_qty_attr, [], 'no-margin') .
			render_input($name_unit_name, '', $unit_name, 'text', ['placeholder' => _l('unit'), 'readonly' => true], [], 'no-margin', 'input-transparent text-right wh_input_none') .
			'</td>';

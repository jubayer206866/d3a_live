...
if (!function_exists('check_csrf_protection')) {
    ...
    function get_customer_name_with_project($id)
    {

        $CI = &get_instance();
        $CI->db->join(db_prefix() . 'projects', db_prefix() . 'projects.clientid=' . db_prefix() . 'clients.userid');
        $CI->db->where('id', $id);
        $data = $CI->db->get(db_prefix() . 'clients')->result_array();
        return $data[0]['company'];
    }
}
function get_packing_list_cbm_summary($packing_list_id)
{
    $CI =& get_instance();

    $CI->db->select_sum('total_cbm');
    $CI->db->where('packing_list_id', $packing_list_id);
    $query = $CI->db->get(db_prefix() . 'wh_packing_list_details');

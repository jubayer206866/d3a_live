...
$(function () {
  ...
  $("body").on("click", ".transaction-submit-custom", function () {
    var that = $(this);
    var form = that.parents("form._transaction_form-custom");
    if (form.valid()) {
      if (that.hasClass("save-as-draft")) {
        form.append(hidden_input("save_as_draft", "true"));
      } else if (that.hasClass("save-and-send")) {
  ...
  $("body").on("submit", "._transaction_form-custom", function () {
    // Add disabled to submit buttons
    $(this).find(".transaction-submit-custom").prop("disabled", true);
    return true;
  });
  ...
  $("body").on("change", 'select[name="invoice_currency"]', function () {
  ...
  $("body").on("change", ".f_client_id #clientid", function () {
    ...
    requestGetJSON(
      ...
      var s_currency = $("body").find(
        '.accounting-template select[name="invoice_currency"]'
      );
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
  ...
  get_taxes_dropdown_template(tax_name, data.taxname).done(function (
    ...
    table_row +=
      '<td class="hide"><textarea name="newitems[' +
      item_key +
    ...
    table_row +=
      '<td><input type="text"  data-quantity name="newitems[' +
      item_key +
      '][unit]" value="' +
      data.unit +
      '" class="form-control">';
...
function init_invoice_currency() {
  var selectedCurrencyId = $('select[name="invoice_currency"]').val();
  requestGetJSON("misc/get_currency/" + selectedCurrencyId).done(function (
    currency
  ) {
    // Used for formatting money
      ...
      ajax: {
        url:
          typeof url == "undefined"
            ? admin_url + "misc/get_relation_data_invoice"
            : url,
        data: function () {
          var data = {};
          data.type = type;

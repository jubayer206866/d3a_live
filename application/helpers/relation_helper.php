...
function get_relation_data_invoice($type, $rel_id = '', $extra = [])
{
    $CI = & get_instance();
    $q  = '';
    if ($CI->input->post('q')) {
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        if ($rel_id != '') {
            ...
            if ($CI->input->post('customer_id')) {
                $client_id = $CI->db->escape_str($CI->input->post('customer_id'));

                $po_projects = $CI->db->select('project')
                ...
                $po_project_ids = array_column($po_projects, 'project');

                foreach($po_project_ids as $pid) {
                    $exists = false;
                    foreach($data as $p) { 
                        if($p['id'] == $pid) $exists = true;
                    }
                    if(!$exists){
                        $p_data = $CI->db->get_where(db_prefix().'projects', ['id'=>$pid])->row_array();

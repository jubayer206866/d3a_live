...
function app_init_project_tabs()
{
    ...
    $CI->app_tabs->add_project_tab('project_packing_list', [
        'name'     => _l('packing_list'),
        'icon'     => 'fa fa-inbox menu-icon',
        'view'     => 'admin/projects/packing_list',
        'position' => 8,
        'visible'  => true,
    ]);
    ...
    // ]);
    $CI->app_tabs->add_project_tab('project_invoices', [
        'name'     => _l('project_invoices'),
        'icon'     => 'fa fa-clipboard menu-icon',
        'view'     => 'admin/projects/project_invoices',
        'position' => 35,
        'visible'  => staff_can('view', 'invoices') || staff_can('view_own', 'invoices'),
    ]);
    ...
    // ]);
    $CI->app_tabs->add_project_tab('project_expenses', [
        'name'     => _l('project_expenses'),
    ...
    $CI->app_tabs->add_project_tab('purchase_order', [
        'name'     => _l('purchase_order'),
        'icon'     => 'fa fa-cart-plus menu-icon',
        'view'     => 'admin/projects/po_on_project_manage',
        'position' => 42,
        'visible'  => true,
    ]);

    $CI->app_tabs->add_project_tab('inventory_receiving_voucher', [
        'name'     => _l('Inventory Received'),
        'icon'     => 'fa fa-object-group menu-icon',
        'view'     => 'admin/projects/manage_purchase_manage',
        'position' => 43,
        'visible'  => true,
    ]);
...
{
    // Always show these tabs regardless of project settings
    $always_visible_tabs = [
        'project_overview',
        'project_packing_list',
        'project_files'
    ];

    foreach ($always_visible_tabs as $t) {
        $applied_settings[$t] = 1;
    }
...
function count_project_status($status, $client_id)
{
    $ci =& get_instance();
    $ci->load->database();

    $ci->db->select('project');
    $ci->db->from(db_prefix() . 'pur_orders');
    $ci->db->where('clients', $client_id);
    $pur_orders_projects = $ci->db->get()->result_array();
    ...
    if (!empty($project_ids)) {
        $ci->db->or_where_in('id', $project_ids);
    }
    $ci->db->group_end();

...
function get_invoice_total_left_to_pay_invoice_ammount($id)
{
    $CI = &get_instance();
    $CI->db->select('invoice_amount')
        ->where('id', $id);
    $invoice_total = $CI->db->get(db_prefix() . 'invoices')->row()->invoice_amount;
    ...
    $totalPayments = 0;
    foreach ($payments as $payment) {
        $totalPayments += $payment['invoice_amount'];
    }
...
function update_invoice_status($id, $force_update = false, $prevent_logging = false)
{
    ...
    if (
        ($original_status == Invoices_model::STATUS_DRAFT && $force_update == false)
...
function user_can_view_invoice($id, $staff_id = false)
{
    ...
    if (
        (has_permission('invoices', $staff_id, 'view_own') && $invoice->addedfrom == $staff_id)
...
function get_client_name_from_invoice($invoice_id)
{
    $CI = &get_instance();


    $CI->db->from(db_prefix() . 'invoices');
    $CI->db->join(db_prefix() . 'clients', '' . db_prefix() . 'clients.userid = ' . db_prefix() . 'invoices.clientid', 'left');
    $CI->db->where('tblinvoices.id', $invoice_id);
    $invoice = $CI->db->get()->result_array();
    return $invoice[0]['company'];
}

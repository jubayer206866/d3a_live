...
// Function to get dates from select range
function get_dates_from_select_range($key)
{
    $today = date('Y-m-d');
    switch ($key) {
        case 'this_month':
        ...
        case '3':
            return ['from' => date('Y-m-d', strtotime('-3 months')), 'to' => $today];
        case '6':
            return ['from' => date('Y-m-d', strtotime('-6 months')), 'to' => $today];
        case '12':
            return ['from' => date('Y-m-d', strtotime('-12 months')), 'to' => $today];
        default:
            return ['from' => '', 'to' => ''];
    }
}

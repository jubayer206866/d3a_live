...
function pdf_third_row($left, $middle, $right, $pdf, $left_width = 40, $middle_width = 40)
{
    $page_start = $pdf->getPage();
    $y_start    = $pdf->GetY();
    ...
    $pdf->MultiCell($middle_width, 0, $middle, 0, 'R', 0, 3, $pdf->GetX() + $left_width - 10, $y_start, true, 0, true);
    ...
    // write the right cell
    $pdf->MultiCell(0, 0, $right, 0, 'R', 0, 1, $pdf->GetX() + $left_width + $middle_width, $y_start, true, 0, true);
    ...
    $pdf->setPage(max($page_end_1, $page_end_2, $page_end_3));
    $pdf->SetXY($pdf->GetX(), $ynew);

    }

    function pdf_logo_url_custom()
    {
        $custom_pdf_logo_image_url = get_option('custom_pdf_logo_image_url');
        $companyUploadPath         = get_upload_path_by_type('company');
        $logoUrl                   = '';
        ...
        if ($logoUrl != '') {
            $logoImage = '<img width="155px" height="40px"  src="' . $logoUrl . '">';
        }

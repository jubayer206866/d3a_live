...
class Invoices_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        $this->db->join(db_prefix() . 'currencies', '' . db_prefix() . 'currencies.id = ' . db_prefix() . 'invoices.invoice_currency', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
            ...
            if ($invoice) {
                $invoice->total_left_to_pay = get_invoice_total_left_to_pay($invoice->id, $invoice->total);
                $invoice->total_left_to_pay_invoice_amount = get_invoice_total_left_to_pay_invoice_ammount($invoice->id);

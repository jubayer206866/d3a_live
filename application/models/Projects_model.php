...
class Projects_model extends App_Model
{
    ...
    {
        if (
            total_rows(db_prefix() . 'pinned_projects', [
    ...
    public function get($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            // Add by Rifat...............
            $this->db->select(db_prefix() . 'projects.*, ' . db_prefix() . 'shipping_company.name as shipping_company_name');
            $this->db->join(db_prefix() . 'shipping_company', db_prefix() . 'shipping_company.id = ' . db_prefix() . 'projects.shipping_company', 'LEFT');
    ...
    public function mark_as($data)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if (
                isset($data['project_marked_as_finished_email_to_contacts'])
                && $data['project_marked_as_finished_email_to_contacts'] == 1
            ) {
                $template_slug = '';

                switch ($data['status_id']) {
                    case 1:
                        $template_slug = 'project_status_Orders_Confirmed_notification';
                        break;

                    case 3:
                        $template_slug = 'project_status_Goods_Received_notification';
                        break;

                    case 4:
                        $template_slug = 'project_marked_as_finished_to_customer';
                        break;

                    case 5:
                        $template_slug = 'project_status_Container_Loaded_notification';
                        break;
                }

                if ($template_slug != '') {
                    $this->send_project_customer_email($data['project_id'], $template_slug);
                }
            }
    ...
    public function add_discussion($data)
    {
        ...
        if ($insert_id) {
            ...
            $this->log_activity($data['project_id'], '_created_discussion', $data['subject'], $data['show_to_customer']);
    ...
    public function get_project_po_items($id)
    {
        $return = [];
        $shop = [];
        $summary = [];
        ...
        $this->db->select('tblpur_vendor.store_number as vendor_code,tblpur_vendor.company as company,tblwh_packing_lists.id as wh_packing_lists_id,tblgoods_delivery.date_c as date');
        $this->db->where(db_prefix() . 'pur_orders.project', $id);
        ...
        foreach ($wh_packing_lists as $wh_packing_list) {
            if ($wh_packing_list['date']) {
                $data['date'] = $wh_packing_list['date'];
            }

            $this->db->select(db_prefix() . 'wh_packing_list_details.*, ' . db_prefix() . 'items.commodity_barcode, items.rate, goods_delivery_detail.price as gd_price, goods_receipt_id');
            ...
            if (count($list_items)) {
                ...
                $vendor_total_cbm = 0;
                $vendor = array();
                $vendor['name'] = 'Shop Name:' . $wh_packing_list['company'] . ' Shop Nr.:' . $wh_packing_list['vendor_code'];
                $vendor['style'] = 'font-size:14px; font-weight:600;text-align:center;  background-color:#D8D8D8;';
                $vendor['span'] = 'colspan="15" class="shop_no"';
                $shop[$wh_packing_list['wh_packing_lists_id']] = $vendor;
                foreach ($list_items as $list_item) {

                    $items = array();
                    $barcode = isset($list_item['commodity_barcode']) ? trim($list_item['commodity_barcode']) : '';

                    // Generate barcode if it doesn't exist or is invalid
                    if (!empty($barcode) && $barcode !== null && $barcode !== '') {
                        $barcode_file = WAREHOUSE_PRINT_ITEM . md5($barcode) . '.svg';
                        ...
                        if ($needs_regeneration) {
                            // Try to generate barcode and retry if it fails
                            $result = $this->warehouse_model->getBarcode($barcode);
                            if (!$result) {
                                // If generation failed, try once more after a brief delay
                                usleep(100000); // 0.1 second delay
                                $result = $this->warehouse_model->getBarcode($barcode);
                                // Log if still failing
                                if (!$result) {
                                    log_message('error', 'Failed to generate barcode for: ' . $barcode . ' after 2 attempts');
                                }
                            }
                        }
                    }

                    $barcode_hash = md5($barcode);
                    $barcode_img = '';
                    if (!empty($barcode) && $barcode !== null && $barcode !== '') {
                        $barcode_img = '<img src="' . base_url('modules/warehouse/uploads/print_item/' . $barcode_hash . '.svg') . '" alt="SVG Image" class="img-responsive" style="width: 100%; max-width: 200px; margin-top: 20px; padding: 10px;">
                <b>' . $barcode . '</b>';
                    } else {
                        $barcode_img = '<div style="padding: 10px;">No barcode available</div>';
                    }
                    $items[]['name'] = $barcode_img;

                    if (isset($arr_images[$list_item['commodity_code']]) && isset($arr_images[$list_item['commodity_code']][0])) {
                        $items[]['name'] = '<img class="images_w_table" src="' . site_url('modules/purchase/uploads/item_img/' . $arr_images[$list_item['commodity_code']][0]['rel_id'] . '/' . $arr_images[$list_item['commodity_code']][0]['file_name']) . '" alt="' . $arr_images[$list_item['commodity_code']][0]['file_name'] . '" >';
                    } else {
                        $items[]['name'] = '';
                    }
                    $items[]['name'] = $list_item['commodity_name'];
                    $items[]['name'] = $list_item['commodity_long_description'];
                    $items[4]['name'] = $list_item['koli'];
                    if ($list_item['koli'] != $receipt_items[$list_item['commodity_code']]) {
                        $items[4]['class'] = 'yellow';
                    } else {
                        $items[4]['style'] = '';
                    }
                    $vendor_koli += (float) $list_item['koli'];
                    $items[]['name'] = $list_item['cope_koli'];
                    $items[]['name'] = $list_item['total_koli'];
                    $vendor_total_peaces += (float) $list_item['total_koli'];
                    $items[]['name'] = $list_item['gd_price'];
                    $pricetotal = (float) $list_item['gd_price'] * (float) $list_item['total_koli'];
                    $items[]['name'] = '¥' . $pricetotal;
                    $vendor_total_prices += (float) $pricetotal;
                    $items[]['name'] = clean_number($list_item['gross_weight']);
                    $items[]['name'] = clean_number($list_item['total_gross_weight']);
                    $vendor_total_gross_weight += (float) $list_item['total_gross_weight'];
                    $items[]['name'] = clean_number($list_item['net_weight']);
                    $items[]['name'] = clean_number($list_item['total_net_weight']);
                    $vendor_total_net_weight += (float) $list_item['total_net_weight'];
                    $items[]['name'] = clean_number($list_item['cbm_koli']);
                    $items[]['name'] = clean_number($list_item['total_cbm']);
                    $vendor_total_cbm += (float) $list_item['total_cbm'];
                    $return[$wh_packing_list['wh_packing_lists_id']][] = $items;
                }
                $sum = array();
                $sum_head['name'] = 'Total Of the ' . $wh_packing_list['company'];
                $sum_head['style'] = 'font-size:14px; font-weight:600;text-align:center; background-color:#FF7C80;';
                $sum_head['span'] = 'colspan="4"';
                $sum[] = $sum_head;
                ...
                $summary[2]['data'] += (float) $vendor_total_prices;
                $sum[]['name'] = '';
                $sum[]['name'] = clean_number($vendor_total_gross_weight);
                $summary[3]['data'] += $vendor_total_gross_weight;
                $sum[]['name'] = '';
                $sum[]['name'] = clean_number($vendor_total_net_weight);
                $summary[4]['data'] += $vendor_total_net_weight;

                $sum[]['name'] = '';
                $sum[]['name'] = clean_number($vendor_total_cbm);
                $summary[5]['data'] += $vendor_total_cbm;

                foreach ($sum as $sum_key => $s) {
                    if ($sum_key) {
                        $sum[$sum_key]['style'] = 'background-color:#FF7C80;';
                    }
                }
                $return[$wh_packing_list['wh_packing_lists_id']][] = $sum;
            }
        }
        $sum = array();
        $sum_head['name'] = 'Total Of Full Order';
        $sum_head['style'] = 'font-size:14px; font-weight:600;';
        $sum_head['span'] = 'colspan="4"';
        $sum[] = $sum_head;
        $sum[]['name'] = $summary[0]['data'];
        $sum[]['name'] = '';
        $sum[]['name'] = $summary[1]['data'];
        $sum[]['name'] = '';
        $sum[]['name'] = '¥' . $summary[2]['data'];
        $sum[]['name'] = '';
        $sum[]['name'] = clean_number($summary[3]['data']);
        $sum[]['name'] = '';
        $sum[]['name'] = clean_number($summary[4]['data']);
        $sum[]['name'] = '';
        $sum[]['name'] = clean_number($summary[5]['data']);


        foreach ($sum as $sum_key => $s) {
            if ($sum_key) {
                $sum[$sum_key]['style'] = 'background-color:#FFF2CC;';
            }
        }
        $data['return'] = $return;
        $data['summary'] = $sum;
    ...
    public function get_project_invoices($id)
    {
        ...
        foreach ($invoices as $invoice) {
            $format_name = $format_name ? $format_name . ', ' . format_invoice_number($invoice['id']) : format_invoice_number($invoice['id']);
        }
        return $format_name;
    }

    public function get_project_po_orders_items($project_id)
    {
        $this->load->model('warehouse/warehouse_model');
        ...
        $this->db->select('
            ...
            v.store_number
        ');
        $this->db->from('tblpur_order_detail');
        $this->db->join('tblitems', 'tblitems.id = tblpur_order_detail.item_code', 'left');
        $this->db->join('tblpur_orders po', 'po.id = tblpur_order_detail.pur_order', 'left');
        $this->db->join('tblpur_vendor v', 'v.userid = po.vendor', 'left');
        $this->db->where('po.project', $project_id);
        $this->db->order_by('po.vendor', 'ASC');

        $items = $this->db->get()->result_array();
        $images = $this->warehouse_model->item_attachments();
        ...
        foreach ($items as $item) {
            ...
            $vendors[$vendor_id]['company_name'] = $item['vendor_name'];
            $vendors[$vendor_id]['vendor_name'] = 'Shop Name:' . $item['vendor_name'] . ' | Shop Nr.: ' . ($item['store_number'] ?? '-');
            $barcode_html = '<div>No barcode</div>';
            if (!empty($item['commodity_barcode'])) {
                $barcode = trim($item['commodity_barcode']);
                $this->warehouse_model->getBarcode($barcode);

                $barcode_html = '
                    <div style="text-align:center">
                        <img src="' . base_url('modules/warehouse/uploads/print_item/' . md5($barcode) . '.svg') . '" style="max-width:120px">
                        <br><b>' . $barcode . '</b>
                    </div>';
            }

            $img_html = '';
            if (isset($images[$item['item_id']][0])) {
                $img = $images[$item['item_id']][0];
                $img_html = '<img src="' . site_url('modules/purchase/uploads/item_img/' . $img['rel_id'] . '/' . $img['file_name']) . '" style="max-width:90px">';
            }
            ...
            $vendors[$vendor_id]['items'][] = [
                ...
                $item['total_koli'],
                $item['price'],
                '¥' . ((float)$item['total_koli'] * (float)$item['price']),
                clean_number($item['gross_weight']),
                clean_number($item['total_gross_weight']),
                clean_number($item['net_weight']),
                clean_number($item['total_net_weight']),
                clean_number($item['cbm_koli']),
                clean_number($item['total_cbm']),
            ];
        }
        return $vendors;
    }

public function get_project_irv_orders_items($project_id)
{
    $this->load->model('warehouse/warehouse_model');
    ...
    $this->db->select('
        ...
        tblitems.id as item_id,
        tblitems.commodity_barcode,
        tblitems.commodity_code as item_commodity_code,
        tblgoods_receipt_detail.*
    ');
    $this->db->from('tblgoods_receipt_detail');
    $this->db->join(
        'tblgoods_receipt gr',
        'gr.id = tblgoods_receipt_detail.goods_receipt_id',
        'left'
    );
    $this->db->join(
        'tblitems',
        'tblitems.id = tblgoods_receipt_detail.commodity_code',
        'left'
    );
    ...
    foreach ($rows as $row) {
        ...
        if (!isset($irvs[$gr_id])) {
            $irvs[$gr_id] = [
                'irv_code'   => $row['supplier_name'],
                'items'    => []
            ];
        }

        $barcode_html = '<div>No barcode</div>';
        if (!empty($row['commodity_barcode'])) {
            $barcode = trim($row['commodity_barcode']);
            $this->warehouse_model->getBarcode($barcode);
        ...
        $img_html = '';
        if (!empty($row['item_id']) && isset($images[$row['item_id']][0])) {
            $img = $images[$row['item_id']][0];
            $img_html = '<img src="' .
                site_url('modules/purchase/uploads/item_img/' .
                $img['rel_id'] . '/' . $img['file_name']) .
                '" style="max-width:90px">';
        }

        $product_code = !empty($row['item_commodity_code']) ? $row['item_commodity_code'] : '';

        $irvs[$gr_id]['items'][] = [
            $barcode_html,   
            $img_html, 
            $product_code,  
            $row['commodity_long_description'], 
            $row['koli'], 
            $row['cope_koli'],
            $row['total_koli'],   
            clean_number($row['price']),  
            '¥' . ((float)$row['total_koli'] * (float)$row['price']),
            clean_number($row['gross_weight']), 
            clean_number($row['total_gross_weight']),  
            clean_number($row['net_weight']),               
            clean_number($row['total_net_weight']),         
            clean_number($row['cbm_koli']),                 
            clean_number($row['total_cbm']),               
        ];
    }
...
');
        ...
        foreach ($project_stocks as $list_item) {
            ...
            $items[]['name'] = $list_item['commodity_name'];
            $items[]['name'] = $list_item['commodity_long_description'];
            $items[4]['name'] = $list_item['quantity'];
            $items[]['name'] = $list_item['cope_koli'];
            $items[]['name'] = (float) $list_item['quantity'] * (float) $list_item['cope_koli'];
            $items[]['name'] = $list_item['price'];
            $pricetotal = clean_number((float) $list_item['price'] * (float) $list_item['quantity'] * (float) $list_item['cope_koli']);
            $items[]['name'] = '¥' . $pricetotal;
            $items[]['name'] = clean_number($list_item['net_weight']);
            $items[]['name'] = clean_number($list_item['net_weight'] * $list_item['quantity']);
            $items[]['name'] = clean_number($list_item['gross_weight']);
            $items[]['name'] = clean_number($list_item['gross_weight'] * $list_item['quantity']);
            $items[]['name'] = clean_number($list_item['cbm_koli']);
            $items[]['name'] = clean_number($list_item['cbm_koli'] * $list_item['quantity']);
            $return[] = $items;
        }
        return $return;
    }

    public function get_project_expenses_grouped_by_category($project_id, $client_id = null)
    {
        if ($client_id === null && is_client_logged_in()) {
            $client_id = get_client_user_id();
        }

        $this->db->select('
            tblexpenses.category,
            tblcategories.name AS category_name,
            SUM(tblexpenses.amount) AS total_amount
        ');
        $this->db->from(db_prefix() . 'expenses AS tblexpenses');
        $this->db->join(
            db_prefix() . 'expenses_categories AS tblcategories',
            'tblcategories.id = tblexpenses.category',
            'left'
        );
        $this->db->where('tblexpenses.project_id', $project_id);

        if ($client_id !== null) {
            $this->db->where('tblexpenses.clientid', $client_id);
        }

        $this->db->group_by('tblexpenses.category');

        return $this->db->get()->result_array();
    }


    public function get_total_goods_money_client($project_id)
    {
        $client_id = get_client_user_id();

        return $this->db
            ->select('SUM(' . db_prefix() . 'goods_receipt.total_goods_money) AS total_goods_money', false)
            ->from(db_prefix() . 'goods_receipt')
            ->join(
                db_prefix() . 'pur_orders',
                db_prefix() . 'goods_receipt.pr_order_id = ' . db_prefix() . 'pur_orders.id',
                'left'
            )
            ->where(db_prefix() . 'goods_receipt.project', $project_id)
            ->where(db_prefix() . 'pur_orders.clients', $client_id)
            ->get()
            ->row_array();
    }
}

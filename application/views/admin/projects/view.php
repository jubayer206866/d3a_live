...
<script>
    ...
    $('#new_project_expense #amount_2').closest('.form-group').hide();

    function toggleAmount2FieldModal() {
        var selectedCategory = $('#new_project_expense select[name="category"]').val();

        if (selectedCategory == '1' || selectedCategory == '3') {
            $('#new_project_expense #amount_2').closest('.form-group').show();
        } else {
            $('#new_project_expense #amount_2').closest('.form-group').hide();
        }
    }

    $('#new_project_expense select[name="category"]').on('change', function() {
        toggleAmount2FieldModal();
    });

    $('#new_project_expense').on('shown.bs.modal', function () {
        toggleAmount2FieldModal();
    });
});
</script>

        ...
        <div class="tw-max-w-4xl tw-mx-auto">
            ...
            <div class="panel_s">
                <div class="panel-body">
                    <div class="horizontal-scrollable-tabs panel-full-width-tabs hide">
            ...
            <?= render_input('name', 'project_name', $value); ?>
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group select-placeholder">
                                        <label for="billing_type"><?= _l('project_billing_type'); ?></label>
                                        <select name="billing_type" class="selectpicker" id="billing_type" data-width="100%" <?= $disable_type_edit; ?>>
                                            <option value="1" <?= (isset($project) && $project->billing_type == 1) ? 'selected' : ''; ?>>
                                                <?= _l('project_billing_type_fixed_cost'); ?>
                                            </option>
                                            <option value="2" <?= (!isset($project) || (isset($project) && $project->billing_type == 2)) ? 'selected' : ''; ?>>
                                                Percentage
                                            </option>
                                        </select>
                                        <?php if ($disable_type_edit != ''): ?>
                                            <p class="text-danger tw-mt-1"><?= _l('cant_change_billing_type_billed_tasks_found'); ?></p>
                                        <?php endif; ?>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group select-placeholder">
                                        <label for="shipping_company"><?= _l('shipping_company'); ?></label>
                                        <div class="clearfix"></div>
                                        <select name="shipping_company" id="shipping_company" class="selectpicker" data-width="100%"
                                            data-none-selected-text="<?= _l('dropdown_non_selected_tex'); ?>">
                                            <option value=""></option>
                                            <?php foreach ($shipping_companies as $company) { ?>
                                                <option value="<?= e($company['id']); ?>"
                                                    <?php
                                                    if (isset($project) && $project->shipping_company == $company['id']) {
                                                        echo 'selected';
                                                    } ?>><?= e($company['name']); ?>
                                                </option>
                                            <?php } ?>
                                ...
                                <!-- Add by Rifat.............(Create Container Number) -->
                                <div class="row">
                                    <div class="col-md-6">
                                        <?= render_input('create_container_number', 'create_container_number', $create_container_number, 'text'); ?>
                                    </div>
                                </div>
                                ...
                                ?>
                                <div class="row hide"><!-- Add by Rifat........... -->
                                    <div class="col-md-6"><!-- Add by Rifat........... -->
                                        <div id="project_cost"
                                            class="<?= e($input_field_hide_class_total_cost); ?>">
                                            <?php $value = (isset($project) ? $project->project_cost : ''); ?>
                                            <?= render_input('project_cost', 'Service Charge', $value, 'number'); ?>
                                        </div>
                                    </div>
                                    <!-- Add by Rifat........... -->
                                    <div class="col-md-6">
                                        <?php $inland_transport = (isset($project) ? $project->inland_transport : '');?>
                                        <?= render_input('inland_transport', 'inland_transport', $inland_transport, 'number'); ?>
                                    </div>
                                </div>
                                <?php
                                ...
                                <div class="col-md-6">
                                    <?php $bill_of_landing_number = (isset($project) ? $project->bill_of_landing_number : ''); ?>
                                    <?= render_input('bill_of_landing_number', 'bill_of_landing_number', $bill_of_landing_number, 'text'); ?>
                                </div>
                                ...
                                <?php if (is_email_template_active('assigned-to-project')) { ?>
                                    <div class="checkbox checkbox-primary tw-mb-0 hide">
                            ...
                            <div role="tabpanel" class="tab-pane hide" id="tab_settings">
                                <div id="project-settings-area">
                                    ...
                                    <?php foreach ($settings as $setting) {
                                        ...
                                        <?php } else { ?>
                                            ...
                                            <div class="form-group mtop15 select-placeholder project-available-features hide">
                                                <label for="available_features"><?= _l('visible_tabs'); ?></label>
                                                <select
                                                    name="settings[available_features][]"
                                                ...
                                                >
                                                    <?php foreach (get_project_tabs_admin() as $tab) {
                                                        if (isset($tab['collapse'])) { ?>
                                                            <optgroup label="<?= e($tab['name']); ?>">
                                                                <?php foreach ($tab['children'] as $tab_dropdown) {
                                                                    $selected = in_array($tab_dropdown['slug'], $default_selected_tabs) ? ' selected' : ''; ?>
                                                                    <option 
                                                        ...
                                                        <?php } else {
                                                            $selected = in_array($tab['slug'], $default_selected_tabs) ? ' selected' : '';
                                                            ?>
                                                            <option 
                                                                value="<?= e($tab['slug']); ?>" 
                                                                <?= $tab['slug'] === 'project_overview' ? 'disabled selected' : '' ?>
                                                                <?= e($selected); ?>
                                                            >
                                                                <?= e($tab['name']); ?>
                                                            </option>
    ...
    <script>
        ...
        $(function() {
            ...
            $clientSelect.on('changed.bs.select', function() {
                ...
                // Auto-generate project name for new projects only
                <?php if (!isset($project)) { ?>
                var clientId = $clientSelect.selectpicker('val');
                var $projectNameInput = $('input[name="name"]');
                
                if (clientId && clientId != '') {
                    // Only auto-generate if the field is empty or hasn't been manually edited
                    var currentValue = $projectNameInput.val();
                    var isAutoGenerated = $projectNameInput.data('auto-generated');
                    
                    if (!currentValue || isAutoGenerated) {
                        $.ajax({
                            url: admin_url + 'projects/generate_project_name',
                            type: 'POST',
                            dataType: 'json',
                            data: {
                                client_id: clientId
                            ...
                            success: function(response) {
                                if (response && response.success) {
                                    ...
                                    $projectNameInput.data('auto-generated', true);
                                }
                            },
            ...
            <?php if (! isset($project)) { ?>
                ...
                // Track if project name was auto-generated
                $('input[name="name"]').on('input', function() {
                    // If user manually edits, remove auto-generated flag

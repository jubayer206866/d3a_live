...
class Clients extends AdminController
{
    ...
    public function index()
    {
        ...
        $shipping_marks = array_filter(array_column($data['clients'], 'shipping_mark'));
        $data['shipping_marks'] = array_map(function($mark) {
            return ['shipping_mark' => $mark];
        }, array_values(array_unique($shipping_marks)));
        
        $tiers = array_filter(array_column($data['clients'], 'tier'));
        $normalized_tiers = array_map('trim', array_map('strval', $tiers));
        $unique_normalized_tiers = array_unique($normalized_tiers);
        $data['tier'] = array_map(function($t) {
            return ['tier' => $t];
        }, array_values($unique_normalized_tiers));
    ...
    public function client($id = '')
    {
        ...
        if ($this->input->post() && !$this->input->is_ajax_request()) {
            if ($id == '') {
                ...
                if (isset($post['address'])) {
                    $post['billing_street']  = $post['address'];
                    $post['shipping_street'] = $post['address'];
                }

                $success = $this->clients_model->update($post, $id);
                if ($success == true) {
        ...
        if ($id == '') {
            ...
            $data['customer_tabs']['stock'] = [
                'slug' => 'stock',
                'name' => _l('Stock'),
                'icon' => 'fa-solid fa-boxes-stacked',
                'view' => 'admin/clients/groups/stock',
                'position' => 30,
                'badge' => [],
                'href' => '#',
                'children' => [],
            ...
            if ($group == 'profile') {
                ...
                $data['clients']    = $this->statement_model->get_client_by_id($id);
                $data['projects']   = $this->statement_model->get_projects_by_client($id);
                $data['statements'] = $this->statement_model->get_client_statement($id);

                $data['title'] = _l('Statements');
                $data = array_merge(
                    $data,
                    prepare_mail_preview_data('customer_statement', $id)
                );
            } elseif ($group == 'map') {
                ...
                $this->db->select('tblgoods_delivery.id as goods_delivery_id, tblgoods_receipt.id as goods_receipt_id,');
                $this->db->from('tblgoods_delivery');
                $this->db->join('tblgoods_receipt', 'tblgoods_receipt.goods_delivery_id = tblgoods_delivery.id', 'left');
                $this->db->where('goods_delivery.customer_code', $id);
                ...
                foreach ($result as $row) {
                    ...
                    foreach ($goods_delivery_details as $goods_delivery_detail) {
                        if (isset($filter_goods_recepits[$goods_delivery_detail['commodity_code']]) && $filter_goods_recepits[$goods_delivery_detail['commodity_code']]['koli'] > $goods_delivery_detail['koli']) {
                            ...
                            $goods_receipt_details_id = $filter_goods_recepits[$goods_delivery_detail['commodity_code']]['id'];
                            if (isset($filter_other_stocks[$goods_receipt_details_id])) {
                                $single['quantity'] = $single['quantity'] - $filter_other_stocks[$goods_receipt_details_id]['quantity'];
                            }
                    ...
                    $data['single_product'] = $item_array;
                }
            }
    ...
    public function statement_pdf()
    {
        ...
        $all_statements = $this->statement_model->get_client_statement($customer_id);
        ...
        if (empty($statements) && !empty($all_statements)) {
            $statements = $all_statements;
        }

        $from_sql = !empty($from) ? to_sql_date($from) : '1970-01-01';
        $to_sql   = !empty($to) ? to_sql_date($to) : date('Y-m-d');

        $summary = $this->statement_model->get_statement($customer_id, $from_sql, $to_sql);
        ...
        $pdf->Output(slug_it(_l('customer_statement') . '-' . $client->company) . '.pdf', $type);
    }
    ...
    public function statement()
    {
        ...
        $this->load->model('statement_model');
        $data['statements'] = $this->statement_model->get_client_statement($customer_id);
    ...
    public function copy_info($client_id)
    {
        ...
        $contact_email = $this->db->select('email')
        ...
        if (!empty($client->country)) {
            $country = $this->db->select('short_name')
        ...
        $info  = "Company Name: {$client->company}<br>";
        $info .= "VAT Number: {$client->vat}<br>";
        $info .= "Address: {$client->address}<br>";
        $info .= "City: {$client->city}<br>";
        $info .= "Zip Code: {$client->zip}<br>";
        $info .= "Country: {$country_name}<br>";
        $info .= "Email: " . ($client->email ?: $contact_email) . "<br>";
        $info .= "Phone Number: {$client->phonenumber}<br>";

        $info_html  = "<pre id='client-info' style='white-space: pre-wrap;'>{$info}</pre>";
        $info_html .= "<button id='copy-btn'>Copy info</button>";

        $info_html .= <<<JS
            <script>
            document.getElementById('copy-btn').addEventListener('click', function() {
                const infoText = document.getElementById('client-info').innerText;

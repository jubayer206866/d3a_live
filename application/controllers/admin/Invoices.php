...
class Invoices extends AdminController
{
    ...
    public function record_payment()
    {
        ...
        if ($this->input->post()) {
            ...
            $payments = $this->payments_model->get_invoice_payments($data['invoiceid']);
            $inv_amount['total'] = 0;
            if (count($payments)) {
                foreach ($payments as $payment) {
                    $inv_amount['total'] += $payment['amount'];
                }
            }
            $inv_amount['total'] += ($data['amount'] / $data['invoice_amount']) * $invoice->total_left_to_pay_invoice_amount;
            $this->db->where('id', $data['invoiceid']);
            $this->db->update(db_prefix() . 'invoices', $inv_amount);

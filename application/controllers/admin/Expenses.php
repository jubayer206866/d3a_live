...
class Expenses extends AdminController
{
    ...
    public function expense($id = '')
    {
        ...
        $data['categories_by_type'] = [
            'operational' => $this->expenses_model->get_category_by_type('operational'),
            'customer'    => $this->expenses_model->get_category_by_type('customer'),
        ];
        $data['title']      = $title;
    ...
    public function get_project_clients($project_id = '')
    {
        ...
        $pur_orders_clients = $this->db->get(db_prefix() . 'pur_orders')->result_array();
        ...
        foreach ($pur_orders_clients as $client) {
            ...
            if ($client_data) {
                $options .= '<option value="' . $client_data->userid . '">' . e($client_data->company) . '</option>';
            }
        }

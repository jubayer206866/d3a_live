...
class Settings extends AdminController
{
    ...
    {
        if (!staff_can('view', 'shipping_company')) {
            access_denied('shipping_company');
        }

        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('shipping_company');
            return;
        }

        $data['title'] = _l('shipping_company');
        $this->load->view('admin/settings/shipping_company', $data);
    }

    //Add By Rifat......
    public function table()
    {
        echo $this->app->get_table_data('shipping_company');
    }

    //Add By Rifat......
    public function shipping_company_add()
    {
        ...
        $this->db->insert(db_prefix() . 'shipping_company', $data);
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Failed to add shipping company'
            ]);
        }
    }

    //Add by Rifat.........
    public function shipping_company_delete($id)
    {
        if (!$id) {
        ...
        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'shipping_company');
        if ($this->db->affected_rows() > 0) {
            set_alert('success', 'Shipping company deleted successfully');
        } else {
            set_alert('danger', 'Failed to delete shipping company');
        }
        redirect(admin_url('Settings/shipping_company'));
    }

    //Add by Rifat.........
    public function shipping_company_edit($id)
    {
        if ($this->input->post()) {
            $data = [
                'name' => $this->input->post('name'),
            ...
            $this->db->where('id', $id);
            $updated = $this->db->update(db_prefix() . 'shipping_company', $data);

            if ($updated) {
                $response = ['success' => true, 'message' => 'Shipping company updated successfully'];
            } else {
    ...
    //Add by Rifat.........
    public function get_shipping_company($id)
    {
        $company = $this->db->get_where(db_prefix() . 'shipping_company', ['id' => $id])->row();

        if ($company) {
            echo json_encode(['success' => true, 'data' => $company]);
        } else {
            echo json_encode(['success' => false, 'message' => 'Shipping company not found']);
        }
    }
}

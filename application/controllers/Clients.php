...
class Clients extends ClientsController
{
    ...
    public function index()
    {
        ...
        $data['project_statuses'] = $this->projects_model->get_project_statuses($client_id);
    ...
    public function projects($status = '')
    {
        ...
        $this->db->where_in('clients', $client_ids);
        $pur_orders_projects = $this->db->get()->result_array();

        $project_ids = [];
        foreach ($pur_orders_projects as $po) {
            if (!empty($po['project'])) {
                $project_ids[] = $po['project'];
            }
        }
        ...
        if (!empty($project_ids)) {
            $this->db->or_where_in('id', $project_ids);
        }
        $this->db->group_end();
        ...
        if (is_numeric($status)) {
            ...
            $status_ids = array_column($data['project_statuses'], 'id');
            $this->db->where_in('status', $status_ids);
        }
    ...
    public function project($id)
    {
        ...
        // ]);
        $project = $this->projects_model->get($id); //Clients permission brack
        ...
        if ($group != 'edit_task') {
            if ($group == 'project_overview') {
                ...
                $goods = $this->projects_model->get_total_goods_money_client($id);
                $data['total_goods_money'] = (float) ($goods['total_goods_money'] ?? 0);
                ...
                foreach ($data['project_expenses_by_category'] as $row) {
                    $data['total_expense_amount'] += (float) $row['total_amount'];
                }

                $data['total_project_value']
                    = $data['total_goods_money'] + $data['total_expense_amount'];
            ...
            } elseif ($group == 'inventory_receiving') {
                ...
                $this->db->where(db_prefix() . 'pur_orders.clients', $client_id);
                $this->db->where(db_prefix() . 'pur_orders.project', $id);
                $this->db->order_by(db_prefix() . 'goods_receipt.pr_order_id', 'DESC');
                $data['inventory_receiving'] = $this->db->get()->result_array();
                ...
                $this->db->select('
                    pur_orders.*,
                    pur_orders.pur_order_name as po_description,
                    pur_orders.number as payment_status,
                    pur_vendor.company as vendor_name,
                    projects.name as project_name
                ');
                $this->db->from(db_prefix() . 'pur_orders');
                $this->db->join(db_prefix() . 'pur_vendor', 'pur_vendor.userid = pur_orders.vendor', 'left');
                $this->db->join(db_prefix() . 'projects', 'projects.id = pur_orders.project', 'left');
                $this->db->where('pur_orders.project', $id);
                $client_id = $this->session->userdata('client_user_id');
                $this->db->where('pur_orders.clients', $client_id);
                ...
                $this->db->join(db_prefix() . 'files f', 'f.rel_id = e.id AND f.rel_type="expense"', 'left');
                $this->db->where('e.project_id', $id);
                $this->db->where('e.clientid', get_client_user_id());
                ...
                $data['title'] = _l('project_packing_list');
            }
    ...
    public function contracts()
    {
        ...
        $this->db->from(db_prefix() . 'contact_fields_value');
        $this->db->where('name !=', '');
        $contracts = $this->db->get()->result_array();

        foreach ($contracts as &$c) {
            switch ($c['type']) {
                case 'landing_contract':
                    $c['type_display'] = 'Landing Contract';
                    break;
                case 'lcl_contract':
                    $c['type_display'] = 'LCL Contract';
                    break;
                case 'ddp_contract':
                    $c['type_display'] = 'DDP Contract';
                    break;
                default:
                    $c['type_display'] = $c['type'];
            }
        }
    ...
    public function statement()
    {
        ...
        $data['clients']  = $this->statement_model->get_client_by_id($client_id);
        $data['projects'] = $this->statement_model->get_projects_by_client($client_id);
        $data['statements'] = $this->statement_model->get_client_statement(get_client_user_id());

        $data['title'] = _l('Statements');
        $this->data($data);
        $this->view('statement_report');
        $this->layout();
    }
    ...
    public function statement_pdf()
    {
        ...
        $this->load->model('statement_model');
        $project_rows = $this->statement_model->get_client_statement(get_client_user_id());

        $statements = [];
        if (!empty($from) || !empty($to)) {
            $from_ts = !empty($from) ? strtotime(to_sql_date($from)) : null;
            $to_ts   = !empty($to) ? strtotime(to_sql_date($to)) : null;

            foreach ($project_rows as $row) {
                $row_date = strtotime($row['start_date']);
                if ((is_null($from_ts) || $row_date >= $from_ts) && (is_null($to_ts) || $row_date <= $to_ts)) {
                    $statements[] = $row;
                }
            }
        } else {
            $statements = $project_rows;
        }

        if (empty($statements) && !empty($project_rows)) {
            $statements = $project_rows;
        }

        $data['statement']['statements'] = $statements;
    ...
    public function view_irv($id)
    {
        ...
        $this->data($data);
        $this->view('customer_view_purchase');
        $this->layout();
    ...
    public function purchase_order($id)
    {
        ...
        $data['purchase_order_detail'] = $this->db
            ->where('pur_order', $id)
    ...
    public function view_expenses($id = '')
    {
        ...
        $data['category']      = $this->expenses_model->get_category($expense->category);
        $data['payment_mode']  = $this->payment_modes_model->get($expense->paymentmode);
        $data['title']         = _l('expense') . ' #' . $id;

        $this->data($data);
        $this->view('view_expenses');
        $this->layout();
    }
    ...
    public function stock()
    {
        ...
        $this->db->from(db_prefix() . 'project_stocks');
        ...
        foreach ($other_stocks as $other_stock) {
            $filter_other_stocks[$other_stock['goods_receipt_details_id']] = $other_stock;
        }

        $this->db->select(db_prefix() . 'goods_delivery.id as goods_delivery_id, ' . db_prefix() . 'goods_receipt.id as goods_receipt_id');
        $this->db->from(db_prefix() . 'goods_delivery');
        $this->db->join(db_prefix() . 'goods_receipt', db_prefix() . 'goods_receipt.goods_delivery_id = ' . db_prefix() . 'goods_delivery.id', 'left');
        $this->db->where(db_prefix() . 'goods_delivery.customer_code', $client_id);
        $result = $this->db->get()->result_array();

        $item_array = [];
        foreach ($result as $row) {
            $goods_receipt_details = $this->warehouse_model->get_goods_receipt_detail($row['goods_receipt_id']);
            $goods_delivery_details = $this->warehouse_model->get_goods_delivery_detail($row['goods_delivery_id']);

            $filter_goods_recepits = [];
            ...
            foreach ($goods_delivery_details as $goods_delivery_detail) {
                $code = $goods_delivery_detail['commodity_code'];
                if (isset($filter_goods_recepits[$code]) && $filter_goods_recepits[$code]['koli'] > $goods_delivery_detail['koli']) {
                    $single['commodity_code'] = $code;
                    ...
                    $single['quantity'] = (int) $filter_goods_recepits[$code]['checkd_koli'] - (int) $goods_delivery_detail['koli'];

                    $goods_receipt_details_id = $filter_goods_recepits[$code]['id'];
                    if (isset($filter_other_stocks[$goods_receipt_details_id])) {
                        $single['quantity'] -= $filter_other_stocks[$goods_receipt_details_id]['quantity'];
                    }
        ...
        $data['stock_data'] = $item_array;
        $data['title'] = _l('client_stock_tab');
    ...
    {
        $contract = $this->db->get_where(db_prefix() . 'contact_fields_value', ['id' => $id])->row();

        if (!$contract) {
            show_404();
        }

        $contract->fields = json_decode($contract->fields_value, true);
        ...
        $merge_fields = [
            ...
            '{date}' => $contract->fields['date'] ?? '',
            '{trading_company_name}' => $contract->fields['trading_company_name'] ?? '',
            '{trading_company_address}' => $contract->fields['trading_company_address'] ?? '',
            '{trading_company_vat}' => $contract->fields['trading_company_vat'] ?? '',
            '{trading_company_city}' => $contract->fields['trading_company_city'] ?? '',
            '{trading_company_country}' => $contract->fields['trading_company_country'] ?? '',

            '{lending_company_name}' => $contract->fields['lending_company_name'] ?? '',
            '{lending_company_address}' => $contract->fields['lending_company_address'] ?? '',
            '{lending_company_vat}' => $contract->fields['lending_company_vat'] ?? '',
            '{lending_company_city}' => $contract->fields['lending_company_city'] ?? '',
            '{lending_company_country}' => $contract->fields['lending_company_country'] ?? '',

            '{borrowing_company_name}' => $contract->fields['borrowing_company_name'] ?? '',
            '{borrowing_company_address}' => $contract->fields['borrowing_company_address'] ?? '',
            '{borrowing_company_vat}' => $contract->fields['borrowing_company_vat'] ?? '',
            '{borrowing_company_city}' => $contract->fields['borrowing_company_city'] ?? '',
            '{borrowing_company_country}' => $contract->fields['borrowing_company_country'] ?? '',

            '{lending_amount}' => $contract->fields['lending_amount'] ?? '',
            '{interest}' => $contract->fields['interest'] ?? '',
            '{days}' => $contract->fields['days'] ?? '',
            '{initial_paid_amount}' => $contract->fields['initial_paid_amount'] ?? '',
            '{repayment_date}' => $contract->fields['repayment_date'] ?? '',
            '{late_payment_amount_per_day}' => $contract->fields['late_payment_amount_per_day'] ?? '',
            '{late_payment_days}' => $contract->fields['late_payment_days'] ?? '',
            '{repayment_mode}' => $contract->fields['repayment_mode'] ?? '',
        ];
